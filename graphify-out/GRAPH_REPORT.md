# Graph Report - multi-shop  (2026-09-25)

## Corpus Check
- 1121 files · ~685,741 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 20 file(s) not represented in the graph (top: .xml 11, (none) 4, .example 1)

## Summary
- 7354 nodes · 18352 edges · 327 communities (298 shown, 29 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 1169 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `01b28b30`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- fetchWithCsrf
- Result
- ModalidadEntrega
- getSupabaseClient
- app/page.tsx
- useAdmin
- TgtgPromocion
- app/kitchen/page.tsx
- @supabase/supabase-js
- mesa-orders-client.tsx
- waiter-auth.ts
- useLanguage
- ComplementoGrupo
- tpv-types.ts
- react
- handleResult
- Category
- CobroFlow.tsx
- IClienteRepository
- entities/types.ts
- Rediseño del árbol de Menús Virtuales — Implementation Plan
- (protected)/pedidos/page.tsx
- albaranes/[id]/page.tsx
- EmpresaColores
- database/index.ts
- getAuthAdminUseCase
- pendientes/page.tsx
- compilerOptions
- getMesaSesionUseCase
- tpv/layout.tsx
- initiateRedsysMesaPaymentUseCase.ts
- dependencies
- server-services.ts
- app/layout.tsx
- SupabaseSuperAdminRepository.ts
- Convenciones a seguir (ya presentes en el repo, no las reinventes)
- glovo.service.ts
- Documentación: Notificaciones de Pedidos por Telegram
- supabase-pedido.repository.ts
- getDomainFromHeaders
- api/pedidos/route.ts
- MenuVirtual
- bar/page.tsx
- getMesaOrdersUseCase.ts
- tpv-catalog-ctx.tsx
- @playwright/test
- promociones/route.ts
- Waiter Panel
- proxy.ts
- Landing Page — Etapa 3: UI de admin — Implementation Plan
- use-toast.ts
- delivery/page.tsx
- landing-seccion.use-case.ts
- waiter-catalog-ctx.tsx
- domain/types.ts
- kitchen/layout.tsx
- SupabaseHoldRepository.ts
- empresas-table.tsx
- language-context.tsx
- components.json
- (protected)/layout.tsx
- s3-client.ts
- supabase-stock.repository.ts
- legal/page.tsx
- t
- telegram.service.ts
- MainActivity
- onboard/SKILL.md
- compilerOptions
- CLAUDE.md - Contexto multi_shop
- devDependencies
- cn
- promocion.use-case.ts
- HistorialClient.tsx
- File Structure
- SupabaseExportRepository.ts
- scripts
- LaborControl — Registro Digital de Jornada
- AnalyticsClient.tsx
- logger.ts
- tgtg/enviar/route.ts
- cart-drawer.tsx
- empresa-theme-provider.tsx
- TooGoodToGo — Documentación del subsistema
- InventarioFisicoClient.tsx
- Delight Techniques
- auth-admin.use-case.ts
- PedidoUseCase
- infrastructure/index.ts
- ¿Qué puede hacer este sistema?
- electron/package.json
- tsconfig.typecheck.json
- MesasGrid.tsx
- Implement Animations
- seed-landing-desde-web.ts
- fake-supabase.ts
- Delivery — Glovo Business LaaS + Redsys TPV
- landing-seccion.dto.ts
- next.config.mjs
- audit-log/page.tsx
- waiter/kitchen/page.tsx
- notify-push/index.ts
- slider-banner.tsx
- Introduce Color Strategically
- setup.js
- BcgScatterChart.tsx
- PerfilLaboral
- processRedsysWebhookUseCase
- 6. Buenas prácticas
- env-validation.ts
- package.json
- Design Critique
- Seguridad — multi_shop
- removeSessionItemUseCase.ts
- banner-visibilidad.ts
- SupabaseReviewQueueRepository.ts
- tabla-plantillas/route.ts
- processRedsysWebhookUseCase.ts
- callbacks.ts
- CategoryNav
- Optimistic UI — borrado de item en ticket de mesa y envío de carrito
- Informe de Auditoría de Cumplimiento Legal — TPV Multi-Shop
- Flujo de Pedidos y WhatsApp
- Polish Systematically
- rate-limit.ts
- pedido.ts
- Color & Contrast
- UX Writing
- supabase-empresa.repository.ts
- Carta Digital Multi-idioma
- MainActivity.java
- Implement Adaptations
- landing-switches.tsx
- Compliance Legal — multi-shop
- Acrónimos y Conceptos Normativos
- app/error.tsx
- global-error.tsx
- tpv/error.tsx
- RGPD — Ciclo de vida de datos de clientes
- Improve Copy Systematically
- Simplify the Design
- Spatial Design
- Typography
- LowStockBadge.tsx
- SupabaseTablaPlantillaRepository.ts
- Generate Comprehensive Report
- Amplify the Design
- Optimization Strategy
- Refine the Design
- waiter-pin/route.ts
- Interaction Design
- Hardening Dimensions
- Recogida y Envío a Domicilio para Tiendas — Implementation Plan
- tpv-catalog-db.ts
- capacitor.config.ts
- IStockRepository
- json-ld.ts
- promociones/unsubscribe/route.ts
- Arquitectura y flujo de datos
- LandingSeccion
- WaiterLoginForm
- MostradorClient.tsx
- Responsive Design
- Flujo de Pases y Pausa en `/waiter/pendientes`
- eslint.config.js
- Kitchen Screens — Comparación Completa
- browser-printer.ts
- .mcp.json
- Offline, resiliencia y UI optimista
- InformeZModal.tsx
- valoracion.use-case.ts
- Recogida implícita sin toggle — Implementation Plan
- next
- File Structure
- Waiter — Sistema de Pases por Ítem
- TPV — Cumplimiento Legal y Normativo
- Mejoras de UI y simplificación de recogida — Implementation Plan
- superadmin/page.tsx
- StarRating
- menu-engineering/page.tsx
- Propina (Tip) en Mesa
- MenuPanel.tsx
- getSupabaseAnonClient
- logger
- movimientos/page.tsx
- Motion Design
- QR Session Enforcement — Mesa Ordering
- orders/page.tsx
- main.ts
- Frontend Aesthetics Guidelines
- Ingrediente
- Mesa Ordering — QR Table Ordering
- descuento.use-case.ts
- PWA & Service Worker — Waiter Panel
- postcss.config.mjs
- version/route.ts
- Step 2: Ask UX-Focused Questions
- PdfRenderer.ts
- empresa-config-toggles.spec.ts
- modalidad-entrega.dto.ts
- kitchen/offline/page.tsx
- tpv/offline/page.tsx
- registerManualMesaPaymentUseCase.ts
- waiter/offline/page.tsx
- tenant-backup/index.ts
- (protected)/page.tsx
- laborcontrol/export/route.ts
- db-smoke.spec.ts
- Patrones de API Admin — multi_shop
- extract/SKILL.md
- SupervisorPanel.tsx
- frontend-design/SKILL.md
- kitchen-bar-csrf.spec.ts
- EmpresaPublic
- confirm-pedido/route.ts
- pago-ok/page.tsx
- electron/printer/index.ts
- IAdminRepository.ts
- waiter-login-form.tsx
- normalize/SKILL.md
- mesa-delete-item-overlay.test.ts
- mesa-repository-sessions.test.ts
- sw-network-timeout.test.ts
- mesas-tenant-header-spoofing.spec.ts
- restore/route.ts
- mesa-remove-item.test.ts
- Capacitor Android PDA — Sistema completo
- FichajeEvento
- Database Schema
- SEO Multi-Tenant
- 5. API Endpoints
- Deferred Items ("Para servir después")
- completeCustomPaymentUseCase.ts
- selection/route.ts
- analytics.use-case.ts
- RecetaItem
- Mesa Client Tokens
- Funciones SECURITY DEFINER — Trampas Críticas
- correcciones/route.ts
- holds/route.ts
- 6. Integración con TPV
- Protección CSRF
- Authentication
- pedidos-borrado-pruebas.spec.ts
- chain-hash.ts
- qr-scanner-lazy.test.ts
- AnalyticsPeriodParams
- sw.js
- Complejidad cognitiva — cómo se cerró y cómo no volver
- TPV — Plan: Sistema WAL Local-First y Backup Fiscal
- Agentes Relevantes para multi_shop
- Fichaje Digital — Registro de Jornada
- Documentación: Toggle Comida/Bebidas en el Menú
- TPV — Pases / Marchas en KDS
- Welcome Discount - Sistema de Descuento de Bienvenida
- Contexto del Proyecto
- Sentry — Error Monitoring y Observabilidad
- Stock, Mermas & Inventario Físico — Sistema de Inventario TPV
- TPV Catalog Cache — Navegación Instantánea + Offline Resilience
- Waiter Ticket & Payment UX
- Waiter Validation Flow
- tpv/pedidos/route.ts
- tgtg-reserva-popup.tsx
- useCommandQueue.ts
- vista-mesa.ts
- Food Cost Analytics — Contexto
- Acuerdo de Tratamiento de Datos (DPA) — Modelo Art. 28 RGPD
- DeltaCard.tsx
- analytics-types.ts
- Sistema de Alérgenos por Producto
- Sistema de Compras y Proveedores (SIALTI) — Contexto
- TPV — Sistema de empleados con PIN y permisos
- pin-cache.ts
- Marketing — Flujo del Sistema
- fichajes/page.tsx
- WaiterBanner
- Row Level Security (RLS)
- rentabilidad/page.tsx
- cierre-report-view.tsx
- laborcontrol/offline-queue.ts
- Product
- Diseño: mejoras de UI y simplificación de "recogida" en modalidades de entrega
- Imágenes — subida, optimización y coste
- supabase-mesa-client-token.repository.ts
- empresa
- TiendaFulfillmentSelector.tsx
- Sistema de Complementos por Producto
- HeatmapGrid.tsx
- proxy-autorizacion.test.ts
- Mesa Payments — Pagar en mesa con Redsys
- Atomicidad del Pago Completo (implementada 2026-07-30)
- rls-policy-hygiene.spec.ts
- getPedidoRepository
- secrets-scan.test.ts
- API Routes
- isTokenRevoked
- food-cost/page.tsx
- Estándares y certificaciones de seguridad — referencia
- analitica-vercel.tsx
- Sub-features
- supabase-security-definer.spec.ts
- MenuCategoryVM
- Prompt: Búsqueda de Clientes Potenciales — multi_shop
- perfil-laboral.dto.ts
- fuzz-api-inputs.test.ts
- Skill Registry — multi-shop
- .verifyToken
- anon-realtime-column-privileges.spec.ts
- ref_node_fs
- albaranes-immutable.spec.ts
- laborcontrol-chain.spec.ts
- rgpd-purge-log.spec.ts
- tpv-cobros-inalterabilidad.spec.ts
- tpv-concurrency.spec.ts
- tpv-rls-multitenant.spec.ts
- tpv-turnos-inalterabilidad.spec.ts
- tpv-verifactu-qr.spec.ts
- mermas/page.tsx
- successResponse
- espera/page.tsx
- electron.d.ts
- vercel.json
- usePagoDeMesa

## God Nodes (most connected - your core abstractions)
1. `Result` - 711 edges
2. `t()` - 302 edges
3. `next` - 273 edges
4. `getSupabaseClient()` - 254 edges
5. `useLanguage()` - 181 edges
6. `handleResult()` - 170 edges
7. `validationErrorResponse()` - 163 edges
8. `fetchWithCsrf()` - 161 edges
9. `react` - 159 edges
10. `resolveAdminContextWithEmpresa()` - 159 edges

## Surprising Connections (you probably didn't know these)
- `Task 19: Traducciones para la UI de asignación masiva` --references--> `es()`  [INFERRED]
  docs/superpowers/plans/2026-09-16-menus-virtuales.md → scripts/seed-landing-desde-web.ts
- `Despliegue (Vercel)` --references--> `main()`  [INFERRED]
  README.md → scripts/seed-landing-desde-web.ts
- `Cart Drawer (`cart-drawer.tsx`)` --references--> `DeliveryMethodSelector()`  [INFERRED]
  docs/context/delivery.md → src/components/DeliveryMethodSelector.tsx
- ``DeliveryMethodSelector`` --references--> `DeliveryMethodSelector()`  [INFERRED]
  docs/context/delivery.md → src/components/DeliveryMethodSelector.tsx
- `Task 5: `TiendaFulfillmentSelector` — icono visible, lista de domicilio rediseñada, recogida como línea fija` --references--> `TiendaFulfillmentSelector()`  [INFERRED]
  docs/superpowers/plans/2026-09-15-mejoras-ui-modalidades-entrega.md → src/components/TiendaFulfillmentSelector.tsx

## Import Cycles
- None detected.

## Communities (327 total, 29 thin omitted)

### Community 0 - "fetchWithCsrf"
Cohesion: 0.03
Nodes (80): Secciones completas (superadmin), Task 24: Campo "Orden" en `/admin/menus-virtuales`, 2. Reordenar — drag & drop real con `@dnd-kit`, 4. Feedback — banner inline + diálogo de confirmación, sin `alert`/`confirm`, Contexto, Decisiones (aprobadas visualmente por el usuario — opción A + modal + drag&drop real), Diseño: rediseño del árbol de Menús Virtuales (admin), Fuera de alcance (+72 more)

### Community 1 - "Result"
Cohesion: 0.04
Nodes (11): Admin Orders Panel — Mesa Behavior, `consolidateSesionOrders` logic, Result, CreateEmpleadoTpvDto, EmpleadoTpv, IEmpleadoTpvRepository, IPedidoRepository, pedidoEffectiveDateMs() (+3 more)

### Community 2 - "ModalidadEntrega"
Cohesion: 0.10
Nodes (17): Task 6: Corregir los tests existentes que quedaron inválidos tras la Task 2 y la Task 5, 4.1 CRUD de modalidades — `/api/admin/modalidades-entrega`, 4.2 Catálogo público en el checkout, 4.3 `POST /api/pedidos` — campos nuevos, 4. API, CreateModalidadEntregaDTO, UpdateModalidadEntregaDTO, ModalidadEntregaUseCase (+9 more)

### Community 3 - "getSupabaseClient"
Cohesion: 0.04
Nodes (109): Cambios en APIs para soportar superadmin, ref_crypto, zod, GET(), querySchema, AnonimizarSchema, POST(), GET() (+101 more)

### Community 4 - "app/page.tsx"
Cohesion: 0.11
Nodes (27): Task 1: `shouldBypassLanding` — regla pura de bypass, CartaPageProps, dynamic, generateMetadata(), dynamic, generateMetadata(), Home(), HomeProps (+19 more)

### Community 5 - "useAdmin"
Cohesion: 0.03
Nodes (80): Imagenes — Trampas Criticas, Imágenes, El pipeline: dónde se optimiza cada cosa, Regla de una línea, Componentes, Task 6: Admin form — second `ImageUploader` for `tienda`, Task 7: `BannerSliderManager` (admin) — TDD, Task 8: Wirear `BannerSliderManager` dentro de `EmpresaAparienciaForm` (+72 more)

### Community 6 - "TgtgPromocion"
Cohesion: 0.07
Nodes (15): Reglas de negocio destacadas, CreateTgtgResult, SendEmailsResult, TgtgUseCase, TgtgWithItems, TgtgItem, TgtgPromocion, TgtgReserva (+7 more)

### Community 7 - "app/kitchen/page.tsx"
Cohesion: 0.07
Nodes (36): AnyGroupValue, COUNTDOWN_COLOR, CountdownCard(), CountdownCardProps, formatTimer(), getElapsedMinutes(), getMergedActionLabel(), getTimeColor() (+28 more)

### Community 8 - "@supabase/supabase-js"
Cohesion: 0.05
Nodes (25): @supabase/supabase-js, args, supabase, MesaSesionUseCase, MesaUseCase, IMesaRepository, Mesa, DeferredItem (+17 more)

### Community 9 - "mesa-orders-client.tsx"
Cohesion: 0.08
Nodes (39): agruparYaPagados(), bebidasPrimero(), buildGroupedItems(), buildPaidByMergeKey(), buildRemainingAndPaidMaps(), buildSeleccion(), canDeleteItem(), createMesaBroadcastChannel() (+31 more)

### Community 10 - "waiter-auth.ts"
Cohesion: 0.14
Nodes (12): ref_node_crypto, authSchema, POST(), deriveSalt(), getPinPepper(), getWaiterTokenSecret(), signWaiterToken(), verifyPin() (+4 more)

### Community 11 - "useLanguage"
Cohesion: 0.05
Nodes (69): Task 2: 6 componentes públicos de sección, Task 3: Reescribir `LandingPage`, 7. Público — componente compartido `ProductImageGallery`, Público — `LandingPage` (nueva, renderizada desde `src/app/page.tsx`), CtaCartaSection(), CtaCartaSectionProps, TRAMA_PANEL, altGaleria() (+61 more)

### Community 12 - "ComplementoGrupo"
Cohesion: 0.07
Nodes (19): Task 7: `MenuVirtualUseCase`, ComplementoGrupoUseCase, agruparComplementosPorProducto(), indexarCategoriasComplemento(), porOrden(), ComplementoGrupo, ComplementoOpcion, ProductoComplementoAsignacion (+11 more)

### Community 13 - "tpv-types.ts"
Cohesion: 0.08
Nodes (29): 3. Archivos afectados, dynamic, TpvAnalyticsPage(), TurnoCerrarPage(), Props, AbrirTurnoInput, abrirTurnoUseCase(), CerrarTurnoInput (+21 more)

### Community 14 - "react"
Cohesion: 0.04
Nodes (111): Task 20: Selección múltiple y asignación masiva en `/admin/productos`, Contexto para quien ejecute esto, 2. `src/components/category-nav.tsx`, 3. Crear — diálogo modal en vez de creación inmediata, lucide-react, react, LoginForm(), LoginFormProps (+103 more)

### Community 15 - "handleResult"
Cohesion: 0.06
Nodes (73): Checklist, Arquitectura de capas, GET(), isValidDate(), querySchema, GET(), GET(), querySchema (+65 more)

### Community 16 - "Category"
Cohesion: 0.07
Nodes (22): 1. Retry ante timeouts transitorios de PostgREST, 2. `getCachedMenu` no debe cachear un `Result` fallido, 3. LCP: `SubcategorySection` nunca propagaba `priority`, Menú Público — Cache y Resiliencia (GET /), Props, baseCategorySchema, categoryIdSchema, CreateCategoryDTO (+14 more)

### Community 17 - "CobroFlow.tsx"
Cohesion: 0.06
Nodes (34): LowStockBadge, `yaCobradoCents`, Props, buildQuickAmounts(), CobroEfectivo(), fmt(), NUMPAD, Props (+26 more)

### Community 18 - "IClienteRepository"
Cohesion: 0.07
Nodes (20): clienteIdSchema, CreateClienteDTO, createClienteSchema, idiomaSchema, UpdateClienteDTO, updateClienteSchema, anonymizeEmail(), ClienteUseCase (+12 more)

### Community 19 - "entities/types.ts"
Cohesion: 0.04
Nodes (77): Compliance Regulatorio, Ley 11/2021 — Ley Antifraude (aplicación al módulo de compras), Módulo: Compras y Proveedores (SIALTI), RD 1619/2012 — Reglamento de Facturación (IVA Soportado), Reglamento (CE) 178/2002 — Legislación Alimentaria General (Trazabilidad Sanitaria), addItemToAlbaranUseCase(), buildTrazabilidadError(), isFechaCaducidadValida() (+69 more)

### Community 20 - "Rediseño del árbol de Menús Virtuales — Implementation Plan"
Cohesion: 0.15
Nodes (10): Rediseño del árbol de Menús Virtuales — Implementation Plan, Task 1: Agregar `@dnd-kit`, Task 3: Backend — conteo de productos por nodo, Task 4: Función pura de reordenamiento, Task 5: `NuevoMenuVirtualDialog`, Task 6: `EliminarMenuVirtualDialog`, Task 7: Rediseñar `menus-virtuales/page.tsx`, Task 8: Verificación manual en navegador (+2 more)

### Community 21 - "(protected)/pedidos/page.tsx"
Cohesion: 0.11
Nodes (28): Verificado antes de diseñar, Cliente, comparePedidos(), computePedidoStats(), DeleteAllOrdersDialog(), DeleteOrderDialog(), ESTADO_TRANSLATION_KEYS, getAriaSortValue() (+20 more)

### Community 22 - "albaranes/[id]/page.tsx"
Cohesion: 0.12
Nodes (20): AddItemForm, AlbaranDetailPage(), defaultTaxRate(), emptyAddFormBase, formatEuros(), selectedCatalogoItem(), AlbaranesPage(), EstadoBadge() (+12 more)

### Community 23 - "EmpresaColores"
Cohesion: 0.14
Nodes (9): ColoresFormProps, ConfiguracionPageClientProps, EmpresaThemeProviderProps, UpdateEmpresaDTO, EmpresaUseCase, normalizePhone(), Empresa, EmpresaColores (+1 more)

### Community 24 - "database/index.ts"
Cohesion: 0.05
Nodes (94): DELETE(), GET(), POST(), PUT(), toAdminCategory(), DELETE(), Params, PUT() (+86 more)

### Community 25 - "getAuthAdminUseCase"
Cohesion: 0.08
Nodes (36): GET(), ALLOWED_ROLES, dynamic, LaborControlLayout(), buildDetalleItemsSSR(), CobroPage(), dynamic, Props (+28 more)

### Community 26 - "pendientes/page.tsx"
Cohesion: 0.11
Nodes (27): 4. Lanzar un pase a cocina, formatTimer(), getElapsedMinutes(), getGroupedItems(), getMergedItems(), getOldestCreatedAt(), GroupedPendienteItem, makeCleanupMap() (+19 more)

### Community 27 - "compilerOptions"
Cohesion: 0.11
Nodes (18): compilerOptions, allowJs, esModuleInterop, incremental, isolatedModules, jsx, lib, module (+10 more)

### Community 28 - "getMesaSesionUseCase"
Cohesion: 0.12
Nodes (19): closeSesionSchema, createMesaSchema, deleteMesaSchema, GET(), PATCH(), GET(), getMesaSchema, mesaSchema (+11 more)

### Community 29 - "tpv/layout.tsx"
Cohesion: 0.12
Nodes (18): TpvRolContext, dynamic, metadata, TURNO_OPTIONAL_PREFIXES, AccionesPanel(), ActionIcon(), ActionIconProps, ActionVariant (+10 more)

### Community 30 - "initiateRedsysMesaPaymentUseCase.ts"
Cohesion: 0.13
Nodes (24): paramsSchema, POST(), CommitCustomPaymentInput, CommitCustomPaymentResult, commitCustomPaymentUseCase(), checkPaymentLock(), claimDivisionSlot(), EmpresaCredentials (+16 more)

### Community 31 - "dependencies"
Cohesion: 0.04
Nodes (45): dependencies, autoprefixer, @aws-sdk/client-s3, bcryptjs, @capacitor/android, @capacitor/app, @capacitor/core, @capacitor/preferences (+37 more)

### Community 32 - "server-services.ts"
Cohesion: 0.14
Nodes (20): Menú Público — Cache y Resiliencia (GET /), ref_server_only, AdminLoginPage(), metadata, dynamic, GET(), getIconMimeType(), manifest() (+12 more)

### Community 33 - "app/layout.tsx"
Cohesion: 0.09
Nodes (17): onRequestError, register(), @sentry/nextjs, generateMetadata(), getMimeType(), inter, playfair, viewport (+9 more)

### Community 34 - "SupabaseSuperAdminRepository.ts"
Cohesion: 0.14
Nodes (7): SuperAdminUseCase, EmpresaStats, EmpresaWithStats, ISuperAdminRepository, SuperAdminGlobalStats, EmpresaRow, SupabaseSuperAdminRepository

### Community 35 - "Convenciones a seguir (ya presentes en el repo, no las reinventes)"
Cohesion: 0.11
Nodes (20): Task 12: Translations, Task 2: Claves de traducción nuevas, Task 5: Translations — `productImage2` / `productImage2Help`, Banner con modo slider Implementation Plan, Convenciones a seguir (ya presentes en el repo, no las reinventes), Task 10: Wirear `SliderBanner` en la página pública y en el test de compliance de imágenes, Task 11: Verificación final, Task 1: Migración de base de datos (+12 more)

### Community 36 - "glovo.service.ts"
Cohesion: 0.10
Nodes (28): 2. Cotización de envío, Use Case (`getDeliveryQuoteUseCase`), CreateGlovoOrderInput, CreateGlovoOrderOutput, GetDeliveryQuoteInput, GetDeliveryQuoteOutput, getDeliveryQuoteUseCase(), ApiErrorResponse (+20 more)

### Community 37 - "Documentación: Notificaciones de Pedidos por Telegram"
Cohesion: 0.11
Nodes (18): Columnas de base de datos, Configuración, Documentación: Notificaciones de Pedidos por Telegram, `empresas`, Endpoints, Flujo — Modo Restaurante (subdomain pedidos / takeaway), Flujo — Modo Tienda, `GET /api/orders/status?token={token}` (+10 more)

### Community 38 - "supabase-pedido.repository.ts"
Cohesion: 0.07
Nodes (36): The `from_validation` Flag, Pedido, CartItem, PedidoItem, BarOrderItem, KitchenBarCounts, KitchenItemRecord, KitchenOrderItem (+28 more)

### Community 39 - "getDomainFromHeaders"
Cohesion: 0.09
Nodes (45): Sistema de Camarero — Trampas Criticas, Endpoint público, Incidente 2026-07-31 (auditoría externa, pentest de código) — IDOR cross-tenant en `/api/mesas/*`, Cómo agregar un test de regresión de seguridad nuevo, POST(), POST(), GetDeliveryQuoteSchema, POST() (+37 more)

### Community 40 - "api/pedidos/route.ts"
Cohesion: 0.13
Nodes (22): QR Session Enforcement, checkMesaPaymentLock(), createPedidoSchema, DefaultData, defaultPedidoSchema, EmpresaOrderData, handleDefaultOrder(), handleMesaOrder() (+14 more)

### Community 41 - "MenuVirtual"
Cohesion: 0.09
Nodes (19): Fase 2: asignación masiva desde `/admin/productos`, Task 16: Repositorio — `addProductos` (aditivo, no reemplaza), Task 17: Use case — exponer `addProductos`, Task 18: API route — `POST` aditivo, Task 19: Traducciones para la UI de asignación masiva, Task 21: Verificación manual de la Fase 2, Task 4: Domain entity + repository interface, Task 5: Use case + wiring DI (+11 more)

### Community 42 - "bar/page.tsx"
Cohesion: 0.11
Nodes (32): `cart-drawer.tsx` — de 17 a 3 (2026-08-09), El orden que funcionó en las dos delicadas, Lo que queda (2026-08-09), `waiter/bar` — de 21 a bajo umbral (2026-08-09), applyLeftSwipeDrag(), applyRightSwipeDrag(), applySwipeDragVisuals(), BarPage() (+24 more)

### Community 43 - "getMesaOrdersUseCase.ts"
Cohesion: 0.16
Nodes (18): addToIndexMap(), calculateTotal(), fetchEmpresaSettings(), fetchItemEstados(), fetchPaymentState(), getMesaOrdersUseCase(), isPersonalizedPaymentComplete(), ItemEstadoMaps (+10 more)

### Community 44 - "tpv-catalog-ctx.tsx"
Cohesion: 0.08
Nodes (35): TPV Catalog Cache — Contexto Cliente + Offline, dynamic, ESTADO_CHIP, ESTADO_LABEL, ExportFichajes(), handleExport(), firstDay(), lastDay() (+27 more)

### Community 45 - "@playwright/test"
Cohesion: 0.12
Nodes (4): cabecerasDePrueba(), nuevoContexto(), PlaywrightRequest, @playwright/test

### Community 46 - "promociones/route.ts"
Cohesion: 0.18
Nodes (18): Protección contra XSS en emails, ASUNTOS_PROMO, construirTextoPlanoPromo(), ContenidoPromo, ContextoEnvioPromo, createPromocionSchema, DestinatarioPromo, EmpresaPromo (+10 more)

### Community 47 - "Waiter Panel"
Cohesion: 0.08
Nodes (26): Call Waiter, Cart UX (Waiter Mode), Display in kitchen/bar, Estado del pedido cuando hay comida retenida, `from_validation` — significado del flag, Item Notes (`nota`), Kitchen Page — Swipe Colors (`/kitchen`), Live updates (+18 more)

### Community 48 - "proxy.ts"
Cohesion: 0.23
Nodes (21): `proxy.ts` — de 27 a bajo umbral (2026-08-09), Rutas públicas (sin autenticación), API Routes — resumen, createErrorResponse(), verifyCsrfToken(), addCorsHeaders(), buildCsp(), buildPageResponse() (+13 more)

### Community 49 - "Landing Page — Etapa 3: UI de admin — Implementation Plan"
Cohesion: 0.33
Nodes (5): Context, Fuera de alcance de esta etapa, Landing Page — Etapa 3: UI de admin — Implementation Plan, Task 5: Verificación final, item()

### Community 50 - "use-toast.ts"
Cohesion: 0.11
Nodes (26): @radix-ui/react-toast, Toast, ToastAction, ToastActionElement, ToastClose, ToastDescription, ToastProps, src_components_ui_toast_toastprovider (+18 more)

### Community 51 - "delivery/page.tsx"
Cohesion: 0.19
Nodes (13): DeliveryPage(), dynamic, GET(), PUT(), DELETE(), GET(), POST(), PUT() (+5 more)

### Community 52 - "landing-seccion.use-case.ts"
Cohesion: 0.29
Nodes (6): setActivoLandingSeccionSchema, agruparActivosPorEmpresa(), LandingSeccionUseCase, ILandingSeccionRepository, LandingSeccionActiva, UpsertLandingSeccionData

### Community 53 - "waiter-catalog-ctx.tsx"
Cohesion: 0.14
Nodes (12): Sistema de offline en WebView, metadata, OfflineBanner(), reloadWhenReady(), CatalogResponse, CatalogStatus, useWaiterCatalog(), WaiterCatalogContext (+4 more)

### Community 54 - "domain/types.ts"
Cohesion: 0.10
Nodes (15): RegistrarFichajeOutput, VerificarCadenaUseCase, CreateAuditEntryInput, IAuditRepository, IChainRepository, AuditEntry, ChainAnchor, ChainVerifyResult (+7 more)

### Community 55 - "kitchen/layout.tsx"
Cohesion: 0.14
Nodes (12): Push Notifications (FCM), @capacitor/preferences, @capacitor/push-notifications, metadata, KitchenOfflineBanner(), reloadWhenReady(), KitchenPinGate(), KitchenSwRegistrar() (+4 more)

### Community 56 - "SupabaseHoldRepository.ts"
Cohesion: 0.15
Nodes (9): CrearHoldInput, GestionarHoldUseCase, CreateHoldInput, IHoldRepository, LegalHold, getHoldRepo(), getLcHoldRepo(), mapRow() (+1 more)

### Community 57 - "empresas-table.tsx"
Cohesion: 0.17
Nodes (10): El patrón que ha funcionado en las quince cerradas, colorDePuntuacion(), EmpresaRow, EmpresasTableProps, EmpresaStats, GoogleReviewsField(), ModuloSwitchProps, SeoCell() (+2 more)

### Community 58 - "language-context.tsx"
Cohesion: 0.03
Nodes (95): UI, View Model del menú, @testing-library/react, vitest, metadata, Props, CartDrawer, getCategoryTab() (+87 more)

### Community 59 - "components.json"
Cohesion: 0.11
Nodes (17): aliases, components, hooks, lib, ui, utils, iconLibrary, rsc (+9 more)

### Community 60 - "(protected)/layout.tsx"
Cohesion: 0.10
Nodes (19): Content Security Policy (CSP), CSP Violation Reporting, Directivas vigentes, Estrategia por capas, Headers adicionales, Nonce por request, next-themes, AdminProtectedLayout() (+11 more)

### Community 61 - "s3-client.ts"
Cohesion: 0.60
Nodes (4): @aws-sdk/client-s3, deleteImageFromR2(), getS3Client(), uploadToR2()

### Community 62 - "supabase-stock.repository.ts"
Cohesion: 0.26
Nodes (9): MovimientosResponse, registrarMermaUseCase(), AjustarStockPayload, Merma, MovimientoStock, RegistrarMermaPayload, TipoMovimiento, FindMovimientosOpts (+1 more)

### Community 63 - "legal/page.tsx"
Cohesion: 0.08
Nodes (24): NavItemDef, CheckItemProps, CobroCount, dynamic, getCobroStats(), getLastPurge(), getVerifactuMode(), LastPurge (+16 more)

### Community 64 - "t"
Cohesion: 0.02
Nodes (151): 7. i18n, framer-motion, AdminSidebar(), AdminSidebarProps, computeInitialOpenGroups(), FilterCtx, filterEntry(), GROUP_COLORS (+143 more)

### Community 65 - "telegram.service.ts"
Cohesion: 0.22
Nodes (15): Flujo — Tienda / Recogida con pago online (`pagos_pickup_habilitados`), GET(), tokenSchema, callbackQuerySchema, POST(), rateLimitTracking(), answerCallbackQuery(), buildOrderMessage() (+7 more)

### Community 66 - "MainActivity"
Cohesion: 0.16
Nodes (8): MainActivity, VersionInfo, android.content.Intent, android.os.Bundle, 3. `CookieManager.flush()` en onPause — obligatorio, 8. Update check requiere dominio — usar BuildConfig.DEFAULT_DOMAIN como fallback, Task 5: Admin UI — sin toggle ni formulario de recogida, Override

### Community 67 - "onboard/SKILL.md"
Cohesion: 0.09
Nodes (22): Assess Onboarding Needs, Context Over Ceremony, Contextual Help, Design Onboarding Experiences, Documentation & Help, Empty State Design, Feature Discovery & Adoption, Guided Tours & Walkthroughs (+14 more)

### Community 68 - "compilerOptions"
Cohesion: 0.17
Nodes (11): compilerOptions, esModuleInterop, module, outDir, resolveJsonModule, rootDir, skipLibCheck, strict (+3 more)

### Community 69 - "CLAUDE.md - Contexto multi_shop"
Cohesion: 0.08
Nodes (25): 1. RLS, 2. GRANTs explícitos (obligatorio desde oct 2026 — Supabase Data API, y ahora tambien a nivel de DB), 3. Funcion auxiliar de aislamiento de tenant, 4. Como aplicar la migracion (OBLIGATORIO — nunca MCP suelto), Arquitectura y Capas, Base de Datos (Trampas Comunes), Capacitor Android PDA — Trampas Criticas, CLAUDE.md - Contexto multi_shop (+17 more)

### Community 70 - "devDependencies"
Cohesion: 0.05
Nodes (39): devDependencies, baseline-browser-mapping, @capacitor/cli, cross-env, dotenv, dotenv-cli, electron, electron-builder (+31 more)

### Community 71 - "cn"
Cohesion: 0.03
Nodes (82): UI & Design System (Tailwind v4), `AllergenSelector`, Reglamento (UE) N.º 1169/2011 — Información Alimentaria al Consumidor (Alérgenos), class-variance-authority, @radix-ui/react-dialog, @radix-ui/react-dropdown-menu, @radix-ui/react-slot, metadata (+74 more)

### Community 72 - "promocion.use-case.ts"
Cohesion: 0.22
Nodes (5): CreatePromocionResult, PromocionUseCase, Promocion, IPromocionRepository, SupabasePromocionRepository

### Community 73 - "HistorialClient.tsx"
Cohesion: 0.11
Nodes (27): APARIENCIA, buildAeatUrl(), CobroConfirmado(), buildTicket(), handlePrint(), EstadoConfirmacion, fmt(), CobroRow (+19 more)

### Community 74 - "File Structure"
Cohesion: 0.14
Nodes (14): Contexto para quien ejecute esto, File Structure, Fuera de alcance de esta etapa, Landing Page — Etapa 1: Ruteo + landing mínima — Implementation Plan, Task 10: Verificación final, Task 2: `hasNosotrosContent` / `hasDondeEstamosContent` — reglas puras de visibilidad, Task 4: `LandingHeader`, Task 5: `LandingPage` (+6 more)

### Community 75 - "SupabaseExportRepository.ts"
Cohesion: 0.10
Nodes (21): exceljs, ref_stream, GenerarExportOutput, GenerarExportUseCase, GenerarResumenParcialOutput, GenerarResumenParcialUseCase, IExportRepository, ExportQuery (+13 more)

### Community 76 - "scripts"
Cohesion: 0.08
Nodes (24): scripts, build, build:analyze, build:electron, build:electron:prep, build:electron:rebuild, db:smoke, dev (+16 more)

### Community 77 - "LaborControl — Registro Digital de Jornada"
Cohesion: 0.09
Nodes (22): 10. Verificación de Cadena, 11. Retenciones Legales (Bloqueo RGPD), 12. Perfil Laboral, 1. Propósito, 2. Arquitectura, 3. Base de Datos, 4. Tipos de Evento, 8. Electron (+14 more)

### Community 78 - "AnalyticsClient.tsx"
Cohesion: 0.14
Nodes (21): recharts, AnalyticsClient(), exportPdf(), fetchData(), handleCustomApply(), handlePeriodo(), buildDailySummaryHtml(), calcDesdeHasta() (+13 more)

### Community 79 - "logger.ts"
Cohesion: 0.16
Nodes (11): InitiateRedsysPaymentInput, ErrorModule, ErrorSeverity, ILogErrorRepository, LogErrorData, isValidUUID(), SupabaseLogErrorRepository, ErrorLogger (+3 more)

### Community 80 - "tgtg/enviar/route.ts"
Cohesion: 0.14
Nodes (24): buildTgtgEmailHtml(), CampanaBase, ContextoEnvio, Destinatario, enviarACadaDestinatario(), enviarSchema, getLocaleForLang(), leerPromoIds() (+16 more)

### Community 81 - "cart-drawer.tsx"
Cohesion: 0.04
Nodes (101): `QRScannerGate`, Optimistic UI — borrado de item en mesa y envío de carrito — Implementation Plan, Self-Review, Task 2: Cablear el overlay en la UI del ticket (borrado optimista real), Task 3: `SendingOverlay` — pantalla de transición al enviar el carrito, Task 4: `executeMesaOrder` — cierre instantáneo + rollback (modo mesa), Task 5: `processStandardOrderResponse` — cierre instantáneo + rollback (checkout general), Task 6: Cablear todo en `CartDrawer` (componente) (+93 more)

### Community 82 - "empresa-theme-provider.tsx"
Cohesion: 0.40
Nodes (10): adjustForDarkMode(), adjustForegroundForBackground(), darkenColor(), EmpresaThemeProvider(), getLuminance(), getSystemDarkMode(), isColorDark(), isValidHex() (+2 more)

### Community 83 - "TooGoodToGo — Documentación del subsistema"
Cohesion: 0.06
Nodes (33): 1. Crear campaña, 2. Seleccionar y enviar campañas, 3. Estados de una campaña activa, 4. Vista de campañas — dos secciones acordeón, 5. Acciones en campañas, 6. Historial, Archivos relevantes, Arquitectura de capas (+25 more)

### Community 84 - "InventarioFisicoClient.tsx"
Cohesion: 0.16
Nodes (11): ApiResponse, Ingrediente, Delta, fmt(), Ingrediente, InventarioFisicoClient(), calcDeltas(), handleConfirmar() (+3 more)

### Community 85 - "Delight Techniques"
Cohesion: 0.10
Nodes (20): Appropriate to Context, Assess Delight Opportunities, Celebration Moments, Compound Over Time, Context Gathering (Do This First), Delight Amplifies, Never Blocks, Delight Principles, Delight Techniques (+12 more)

### Community 86 - "auth-admin.use-case.ts"
Cohesion: 0.22
Nodes (8): Anonimización de PII en logs, LoginDTO, loginSchema, anonymizeEmail(), AuthAdminUseCase, IAdminRepository, { logAndReturnErrorMock }, noopRepo

### Community 87 - "PedidoUseCase"
Cohesion: 0.06
Nodes (27): Fase 6 — Hallazgos de la revisión final (post-Task 17), Task 16: Revalidación server-side y persistencia, Task 18: Cerrar C1 (domicilio sin dirección), I3 (RLS sin WITH CHECK), I4 (FK sin ON DELETE), I5 (Zod default bug), Task 19: C2 — Lectura de la modalidad de entrega en el pedido, Task 3: Backend — persistir recogida implícita sin `modalidad_entrega_id`, 1. Backend — persistencia implícita, CreateMesaPedidoDTO, CreatePedidoDTO (+19 more)

### Community 88 - "infrastructure/index.ts"
Cohesion: 0.12
Nodes (27): Resolución de superseded, 13. Trampas Críticas, CRON_SECRET — comparación en tiempo constante, GET(), GET(), KioskSchema, POST(), sugerirTipo() (+19 more)

### Community 89 - "¿Qué puede hacer este sistema?"
Cohesion: 0.11
Nodes (18): Analítica, 💾 Backup Automático, Compras y Proveedores — SIALTI, Cumplimiento legal (Ley Antifraude + RD 1007/2023 + RD 1619/2012), Empleados TPV — autenticación por PIN, ⏱️ Fichaje Digital — LaborControl (Art. 34.9 ET · RD-Ley 8/2019), Food Cost Analytics, Gestión de turno y cobro (+10 more)

### Community 90 - "electron/package.json"
Cohesion: 0.29
Nodes (6): devDependencies, electron-store, node-thermal-printer, electron-store, node-thermal-printer, type

### Community 91 - "tsconfig.typecheck.json"
Cohesion: 0.29
Nodes (6): ./tsconfig.json, compilerOptions, incremental, noEmit, extends, include

### Community 92 - "MesasGrid.tsx"
Cohesion: 0.11
Nodes (15): dynamic, cerrarMesaPagada(), FooterProps, getMesaColors(), MesaColors, MesaFooter(), MesasGrid(), Props (+7 more)

### Community 93 - "Implement Animations"
Cohesion: 0.10
Nodes (19): Accessibility, Assess Animation Opportunities, Context Gathering (Do This First), CSS Animations, Delight Moments, Entrance Animations, Feedback & Guidance, Implement Animations (+11 more)

### Community 94 - "seed-landing-desde-web.ts"
Cohesion: 0.13
Nodes (25): CI (GitHub Actions), Contra qué corre el E2E (importante), Git hooks (Husky), Los 5 tests que se saltan SIEMPRE, y por qué está bien, Suites de test, Testing & CI, absoluta(), construirSecciones() (+17 more)

### Community 95 - "fake-supabase.ts"
Cohesion: 0.13
Nodes (13): libereBloqueo(), marcoSesionPagada(), EMPRESA_OK, glovoSpy, telegramSpy, ClaveRespuesta, ConfigFake, FakeSupabase (+5 more)

### Community 96 - "Delivery — Glovo Business LaaS + Redsys TPV"
Cohesion: 0.07
Nodes (26): 10. Errores relacionados, 1. Zona de cobertura, 3. Creación del pedido con delivery, 4. Pago Redsys, 5. Despacho Glovo, 6. Componentes UI, 7. Variables de entorno requeridas, 8. Configuración en admin (+18 more)

### Community 97 - "landing-seccion.dto.ts"
Cohesion: 0.18
Nodes (10): contenidoSchemaPorTipo, ctaCartaContenidoSchema, galeriaContenidoSchema, heroContenidoSchema, nosotrosContenidoSchema, testimonioContenidoSchema, translatableTextSchema, UpsertLandingSeccionDTO (+2 more)

### Community 98 - "next.config.mjs"
Cohesion: 0.17
Nodes (7): __dirname, imgSrc, mediaSrc, nextConfig, r2Origin, ref_node_url, alias

### Community 99 - "audit-log/page.tsx"
Cohesion: 0.08
Nodes (29): Acciones Instrumentadas, Archivos Clave, Arquitectura, Patrón de Uso en Rutas, Propósito, Sistema de Auditoría (Audit Log), Trampas, ACTION_LABELS (+21 more)

### Community 100 - "waiter/kitchen/page.tsx"
Cohesion: 0.13
Nodes (29): addItemBackIfMissing(), applyKitchenSwipeVisuals(), applyLeftSwipe(), applyRightSwipe(), formatTimer(), getElapsedMinutes(), getItemActionColor(), getItemBadgeStyle() (+21 more)

### Community 101 - "notify-push/index.ts"
Cohesion: 0.22
Nodes (3): ref_jsr_supabase, ref_npm_google_auth_library_9, PushPayload

### Community 102 - "slider-banner.tsx"
Cohesion: 0.33
Nodes (8): directionTo(), nextIndex(), prefersReducedMotion(), previousIndex(), SliderBanner(), goTo(), SliderBannerProps, slideVariants

### Community 103 - "Introduce Color Strategically"
Cohesion: 0.11
Nodes (18): Accent Color Application, Accessibility, Assess Color Opportunity, Background & Surfaces, Balance & Refinement, Borders & Accents, Cohesion, Context Gathering (Do This First) (+10 more)

### Community 104 - "setup.js"
Cohesion: 0.18
Nodes (7): domainInput, emailInput, errorEl, passwordInput, printerSelect, saveBtn, successBanner

### Community 105 - "BcgScatterChart.tsx"
Cohesion: 0.22
Nodes (8): BcgScatterChartProps, LEGEND_ITEMS, QUADRANT_BG, QUADRANT_COLOR, TooltipPayload, TooltipProps, BcgItem, BcgQuadrant

### Community 106 - "PerfilLaboral"
Cohesion: 0.15
Nodes (11): derivarEstado(), ObtenerEstadoSupervisorUseCase, segundosDesde(), CreatePerfilLaboralInput, IPerfilLaboralRepository, UpdatePerfilLaboralInput, EstadoSupervisor, PerfilLaboral (+3 more)

### Community 107 - "processRedsysWebhookUseCase"
Cohesion: 0.27
Nodes (15): aplicarCobroSiProcede(), GET(), POST(), procesarYRedirigir(), OK(), POST(), decodificarParametros(), processRedsysWebhookUseCase() (+7 more)

### Community 108 - "6. Buenas prácticas"
Cohesion: 0.08
Nodes (25): 1. Flujo general de un cobro, 2. Cobro parcial, 3. Detección de cobro externo (Realtime), 4. Historial del turno — selector de turno, 5. Rectificativos — modelo legal y visualización, 6. Buenas prácticas, Campo `iva_porcentaje` graba la tasa histórica, Canal Realtime (+17 more)

### Community 109 - "env-validation.ts"
Cohesion: 0.47
Nodes (4): ENV_VARS, EnvVar, validateEnv(), register()

### Community 110 - "package.json"
Cohesion: 0.04
Nodes (51): electron-store, node-thermal-printer, name, pnpm, onlyBuiltDependencies, private, productName, type (+43 more)

### Community 111 - "Design Critique"
Cohesion: 0.11
Nodes (18): 10. Microcopy & Voice, 1. AI Slop Detection (CRITICAL), 2. Visual Hierarchy, 3. Information Architecture, 4. Emotional Resonance, 5. Discoverability & Affordance, 6. Composition & Balance, 7. Typography as Communication (+10 more)

### Community 112 - "Seguridad — multi_shop"
Cohesion: 0.06
Nodes (32): Aislamiento de tenant en endpoints de mesa pública, Aislamiento multi-tenant, Comportamiento por entorno, CORS, CSP — Vercel preview toolbar, Endpoint de desarrollo protegido en producción, Glovo webhook — HMAC-SHA256, JSON-LD Sanitization (+24 more)

### Community 113 - "removeSessionItemUseCase.ts"
Cohesion: 0.12
Nodes (23): TPV Cobros — IVA/IGIC, Compliance y RGPD, Waiter-Only: Paid Items Section, bodySchema, DELETE(), mesaIdSchema, calcularTotal(), cerrarSesionSiQuedaCubierta(), esElItemBuscado() (+15 more)

### Community 114 - "banner-visibilidad.ts"
Cohesion: 0.17
Nodes (12): `waiter-banner.tsx` — de 16 a 7 (2026-08-09), RootLayout(), ClaveSeccion, ContextoBanner, debeMontarseWaiterBanner(), MotivoOculto, motivoParaOcultarBanner(), Regla (+4 more)

### Community 115 - "SupabaseReviewQueueRepository.ts"
Cohesion: 0.23
Nodes (7): CreateReviewItemInput, IReviewQueueRepository, ReviewEstado, ReviewQueueItem, ReviewTipo, mapRow(), SupabaseReviewQueueRepository

### Community 116 - "tabla-plantillas/route.ts"
Cohesion: 0.15
Nodes (15): DELETE(), GET(), POST(), createProductSchema, imageFitValues, productIdSchema, tablaInfoSchema, updateProductSchema (+7 more)

### Community 117 - "processRedsysWebhookUseCase.ts"
Cohesion: 0.13
Nodes (22): `createGlovoOrderUseCase`, createGlovoOrderUseCase(), aplicarEfectosDelCobro(), cargarEmpresa(), cerrarSesionPagada(), construirPedidoParaTelegram(), DatosCliente, DatosEmpresa (+14 more)

### Community 118 - "callbacks.ts"
Cohesion: 0.13
Nodes (23): botonesRespuestaRapida(), botonesTiempoFijado(), BotonTelegram, cancelarEntregado(), Contexto, escaparMarkdown(), fijarTiempo(), Manejador (+15 more)

### Community 119 - "CategoryNav"
Cohesion: 0.05
Nodes (41): Desplegable de subcategorías en el menú — Implementation Plan, Notas de validación previa (spike ya ejecutado), Task 1: Módulo de reglas — subcategorías con productos, Task 2: Módulo de reglas — origen de la animación del diálogo, Task 3: Polyfills de jsdom para montar CategoryNav/MenuSection en tests, Task 4: Ancla DOM propia por subcategoría en `MenuSection`, Task 5: Traducciones nuevas, Task 6: `CategoryNav` — diálogo de subcategorías (+33 more)

### Community 120 - "Optimistic UI — borrado de item en ticket de mesa y envío de carrito"
Cohesion: 0.14
Nodes (14): Contexto, Flujo 1 — Borrar item del ticket, Flujo 2 — Enviar carrito (mesa + checkout general), Fuera de alcance, Objetivo, Optimistic UI — borrado de item en ticket de mesa y envío de carrito, Patrón: cierre instantáneo + continuación diferida (sin snapshot/restore), Patrón: overlay optimista (no se muta `sessionData`) (+6 more)

### Community 121 - "Informe de Auditoría de Cumplimiento Legal — TPV Multi-Shop"
Cohesion: 0.08
Nodes (25): ACID, Checklist Final, Compliance Inventory — Estado, Compliance Score, Correcciones Post-Auditoría (commit 2abad57), Dictamen Formal, Endpoints de Auditoría, Funciones SQL Críticas (+17 more)

### Community 122 - "Flujo de Pedidos y WhatsApp"
Cohesion: 0.08
Nodes (23): 1. Detección por Subdominio (mecanismo principal), 2. Resolución de Empresa, API Route: `/api/pedidos`, Arquitectura General, Database Schema, Diálogo según plataforma, Documentación del Flujo de Acceso al Carrito y Pedidos, Flujo Completo (+15 more)

### Community 123 - "Polish Systematically"
Cohesion: 0.12
Nodes (16): Code Quality, Color & Contrast, Content & Copy, Edge Cases & Error States, Final Verification, Forms & Inputs, Icons & Images, Interaction States (+8 more)

### Community 124 - "rate-limit.ts"
Cohesion: 0.12
Nodes (22): @upstash/ratelimit, @upstash/redis, cspReportSchema, POST(), sanitizeUri(), DELETE(), paramsSchema, mesaIdSchema (+14 more)

### Community 125 - "pedido.ts"
Cohesion: 0.17
Nodes (15): DELETE(), GET(), getMesaId(), LockRow, mesaIdSchema, POST(), requireMesaInOwnTenant(), ESTADOS_POR_ORIGEN (+7 more)

### Community 126 - "Color & Contrast"
Cohesion: 0.12
Nodes (15): Alpha Is A Design Smell, Building Functional Palettes, Color & Contrast, Color Spaces: Use OKLCH, Contrast & Accessibility, Dangerous Color Combinations, Dark Mode Is Not Inverted Light Mode, Never Use Pure Gray or Pure Black (+7 more)

### Community 127 - "UX Writing"
Cohesion: 0.12
Nodes (16): Avoid Redundant Copy, Confirmation Dialogs: Use Sparingly, Consistency: The Terminology Problem, Don't Blame the User, Empty States Are Opportunities, Error Message Templates, Error Messages: The Formula, Form Instructions (+8 more)

### Community 128 - "supabase-empresa.repository.ts"
Cohesion: 0.12
Nodes (15): Task 4: Repositorio Supabase, Task 3: Repositorio — TDD sobre `construirPayloadEmpresa`, Arquitectura de datos, Capas afectadas, DEFAULT_DESCUENTO_BIENVENIDA_PORCENTAJE, DEFAULT_PEDIDOS_SUBDOMAIN, DESCUENTO_EXPIRACION_DIAS, CAMPOS_DIRECTOS (+7 more)

### Community 129 - "Carta Digital Multi-idioma"
Cohesion: 0.12
Nodes (16): Android — Capacitor (panel camarero), Apps Nativas, Arquitectura — Clean Architecture, Build del ejecutable, Carta Digital Multi-idioma, Comandos, Despliegue (Vercel), Documentación (+8 more)

### Community 130 - "MainActivity.java"
Cohesion: 0.13
Nodes (13): alertdialog, com.getcapacitor.BridgeActivity, cookiemanager, file, fileprovider, httpurlconnection, jsonobject, notificationchannel (+5 more)

### Community 131 - "Implement Adaptations"
Cohesion: 0.13
Nodes (14): Assess Adaptation Challenge, Content Adaptation, Desktop Adaptation (Mobile → Desktop), Email Adaptation (Web → Email), Implement Adaptations, Layout Adaptation Techniques, Mobile Adaptation (Desktop → Mobile), Navigation Adaptation (+6 more)

### Community 132 - "landing-switches.tsx"
Cohesion: 0.22
Nodes (7): LandingSwitches(), LandingSwitchesProps, SeccionSwitch(), SeccionSwitchProps, TIPO_LABELS, LANDING_SECCION_TIPOS, fetchWithCsrf

### Community 133 - "Compliance Legal — multi-shop"
Cohesion: 0.11
Nodes (18): AEAT QR en Ticket, Art.66 LGT — Retención fiscal mínima 5 años, CMP (Coste Medio Ponderado) — sin obligación legal directa, Compliance Legal — multi-shop, Decreto 182/1992 y sucesivos — Impuesto General Indirecto Canario (IGIC), Ley 11/2021 — Informe Z y Número de Serie, Ley 11/2021 — Medidas de Prevención y Lucha contra el Fraude Fiscal (Ley Antifraude), Módulo: Carta Digital (+10 more)

### Community 134 - "Acrónimos y Conceptos Normativos"
Cohesion: 0.08
Nodes (23): Acrónimos y Conceptos Normativos, Arqueo Ciego (Blind Cash Count), Audit Trail (Registro de Auditoría Inalterable), Clean Architecture (Arquitectura Limpia), Estándares de Calidad de Software, Estándares de Seguridad, Factura Rectificativa, Glosario de Términos Legales, Normativos y de Calidad (+15 more)

### Community 138 - "RGPD — Ciclo de vida de datos de clientes"
Cohesion: 0.22
Nodes (8): Actualización de actividad, Alta de cliente, Ciclo de vida completo, Derecho de supresión (Art. 17 RGPD), Preguntas frecuentes, Purga automática (Vercel Cron), Qué datos SE CONSERVAN (obligación fiscal), RGPD — Ciclo de vida de datos de clientes

### Community 139 - "Improve Copy Systematically"
Cohesion: 0.13
Nodes (14): Apply Clarity Principles, Assess Current Copy, Button & CTA Text, Confirmation Dialogs, Empty States, Error Messages, Form Labels & Instructions, Help Text & Tooltips (+6 more)

### Community 140 - "Simplify the Design"
Cohesion: 0.13
Nodes (14): Assess Current State, Code Simplification, Content Simplification, Context Gathering (Do This First), Document Removed Complexity, Information Architecture, Interaction Simplification, Layout Simplification (+6 more)

### Community 141 - "Spatial Design"
Cohesion: 0.13
Nodes (14): Cards Are Not Required, Container Queries, Depth & Elevation, Grid Systems, Hierarchy Through Multiple Dimensions, Name Tokens Semantically, Optical Adjustments, Spacing Systems (+6 more)

### Community 142 - "Typography"
Cohesion: 0.13
Nodes (14): Accessibility Considerations, Choosing Distinctive Fonts, Classic Typography Principles, Fluid Type, Font Selection & Pairing, Modern Web Typography, Modular Scale & Hierarchy, OpenType Features (+6 more)

### Community 143 - "LowStockBadge.tsx"
Cohesion: 0.15
Nodes (13): Optimistic UI, Personalizado, Presets, Quitar propina, Realtime, UI — `TipSelector`, 6. Race condition broadcast vs. auto-cancel en MostradorClient, AlertsResponse (+5 more)

### Community 144 - "SupabaseTablaPlantillaRepository.ts"
Cohesion: 0.17
Nodes (10): PlantillaResumen, CreateTablaPlantillaDTO, propagarError(), TablaPlantillaUseCase, TablaCelda, TablaPlantilla, CreateTablaPlantillaData, ITablaPlantillaRepository (+2 more)

### Community 145 - "Generate Comprehensive Report"
Cohesion: 0.14
Nodes (13): Anti-Patterns Verdict, Critical Issues, Detailed Findings by Severity, Diagnostic Scan, Executive Summary, Generate Comprehensive Report, High-Severity Issues, Low-Severity Issues (+5 more)

### Community 146 - "Amplify the Design"
Cohesion: 0.14
Nodes (13): Amplify the Design, Assess Current State, Color Intensification, Composition Boldness, Context Gathering (Do This First), MANDATORY PREPARATION, Motion & Animation, Plan Amplification (+5 more)

### Community 147 - "Optimization Strategy"
Cohesion: 0.14
Nodes (13): Animation Performance, Assess Performance Issues, Core Web Vitals Optimization, Cumulative Layout Shift (CLS < 0.1), First Input Delay (FID < 100ms) / INP (< 200ms), Largest Contentful Paint (LCP < 2.5s), Loading Performance, Network Optimization (+5 more)

### Community 148 - "Refine the Design"
Cohesion: 0.15
Nodes (12): Assess Current State, Color Refinement, Composition Refinement, Context Gathering (Do This First), MANDATORY PREPARATION, Motion Reduction, Plan Refinement, Refine the Design (+4 more)

### Community 149 - "waiter-pin/route.ts"
Cohesion: 0.20
Nodes (12): DELETE(), PATCH(), PatchSchema, CreateSchema, POST(), pinSchema, POST(), EmpleadoTpvLoginUseCase (+4 more)

### Community 150 - "Interaction Design"
Cohesion: 0.17
Nodes (12): Destructive Actions: Undo > Confirm, Focus Rings: Do Them Right, Form Design: The Non-Obvious, Gesture Discoverability, Interaction Design, Keyboard Navigation Patterns, Loading States, Modals: The Inert Approach (+4 more)

### Community 151 - "Hardening Dimensions"
Cohesion: 0.17
Nodes (11): Accessibility Resilience, Assess Hardening Needs, Edge Cases & Boundary Conditions, Error Handling, Hardening Dimensions, Input Validation & Sanitization, Internationalization (i18n), Performance Resilience (+3 more)

### Community 152 - "Recogida y Envío a Domicilio para Tiendas — Implementation Plan"
Cohesion: 0.13
Nodes (14): Fase 1 — Backend: CRUD de modalidades de entrega, Fase 2 — Admin UI, Fase 5 — Verificación final, Fase 7 — Seguimiento post-lanzamiento (2026-09-15), Recogida y Envío a Domicilio para Tiendas — Implementation Plan, Recomendación de PRs encadenadas, Self-Review, Task 1: Migración SQL (+6 more)

### Community 153 - "tpv-catalog-db.ts"
Cohesion: 0.19
Nodes (12): 5. `removeSessionItemUseCase` bypasea `pedido_item_estados`, Invalidación reactiva del catálogo, buildComplementoMap(), TpvCatalogProvider(), CatalogConfig, CatalogSnapshot, CategoriesRecord, ConfigRecord (+4 more)

### Community 155 - "IStockRepository"
Cohesion: 0.16
Nodes (4): Re-habilitación de productos, ajustarStockUseCase(), rehabilitarProductosSiUmbralSuperado(), IStockRepository

### Community 156 - "json-ld.ts"
Cohesion: 0.19
Nodes (19): 1. Árbol — jerarquía visual clara (opción A del brainstorming), JsonLd(), buildBusinessNode(), buildJsonLdGraph(), buildMenuNode(), buildWebSiteNode(), businessId(), coordenadasValidas() (+11 more)

### Community 157 - "promociones/unsubscribe/route.ts"
Cohesion: 0.22
Nodes (12): emailSchema, GET(), getBaseUrl(), uuidSchema, emailSchema, GET(), getBaseUrl(), uuidSchema (+4 more)

### Community 158 - "Arquitectura y flujo de datos"
Cohesion: 0.14
Nodes (13): 1. Base de datos, 2. Domain (`core/domain/entities/types.ts`), 3. Application, 4. Infrastructure (`SupabaseProductRepository.ts`), 5. Admin (`product-form-dialog.tsx`), 6. Público — View Model, 8. Traducciones, Alcance (+5 more)

### Community 159 - "LandingSeccion"
Cohesion: 0.24
Nodes (4): LandingSeccionApi, LandingSeccion, LandingSeccionTipo, SupabaseLandingSeccionRepository

### Community 160 - "WaiterLoginForm"
Cohesion: 0.09
Nodes (22): WaiterBanner — Re-autenticacion sin recarga, 10. WaiterLoginForm — flash de PIN con sesión válida, 11. No existe `/waiter/mesas`, 12. `android/` está en .gitignore, 1. `npx cap copy android` es obligatorio antes de cada build, 2. `SameSite=strict` bloquea cookies en Capacitor, 4. `style.display = ''` no muestra elementos con CSS display:none, 5. `window.load` no `DOMContentLoaded` para el bridge de Capacitor (+14 more)

### Community 161 - "MostradorClient.tsx"
Cohesion: 0.07
Nodes (39): TPV Mostrador — Trampas Criticas, dynamic, EMPTY_MESA_DATA, loadMesaData(), mapPedido(), MesaData, MostradorPage(), normComplement() (+31 more)

### Community 162 - "Responsive Design"
Cohesion: 0.18
Nodes (10): Breakpoints: Content-Driven, Detect Input Method, Not Just Screen Size, Layout Adaptation Patterns, Mobile-First: Write It Right, Picture Element for Art Direction, Responsive Design, Responsive Images: Get It Right, Safe Areas: Handle the Notch (+2 more)

### Community 163 - "Flujo de Pases y Pausa en `/waiter/pendientes`"
Cohesion: 0.18
Nodes (11): Flujo completo: lanzar pase con ítem pausado, Flujo de Pases y Pausa en `/waiter/pendientes`, Pases (primer / segundo / postre), Pausa en pendientes — semántica del botón Pause, `retainIndices` vs `pausedIndices` — diferencia semántica, Trampas históricas (bugs ya resueltos), Happy path — validate all at once, Partial confirm — comida first, bebida later (+3 more)

### Community 166 - "Kitchen Screens — Comparación Completa"
Cohesion: 0.09
Nodes (23): "¿Cuál es la fuente de verdad del estado de un item?", En `/waiter/pendientes`, Estados y swipe, Estados y swipe, Estados y swipe, `/kitchen` — Pantalla Standalone de Cocina, Kitchen Screens — Comparación Completa, Merge key incluye nota (+15 more)

### Community 167 - "browser-printer.ts"
Cohesion: 0.12
Nodes (23): Archivos fuente, Arquitectura, Electron TPV Windows, Pendiente verificable solo con impresora real, Proceso de build, Seguridad, Trampas, Versiones (+15 more)

### Community 168 - ".mcp.json"
Cohesion: 0.22
Nodes (10): SUPABASE_ACCESS_TOKEN, graphify-mcp, npx, context7, graphify, playwright, supabase, @playwright/mcp (+2 more)

### Community 169 - "Offline, resiliencia y UI optimista"
Cohesion: 0.09
Nodes (22): 1. UI optimista: la interfaz no espera al servidor, 2. Cola de comandos offline, 3. Idempotencia de pedidos (`b571c07`), 4. Resiliencia de Realtime, 5. Por qué hubo que escribir tantos tests, 6. Lo que la suite E2E NO estaba cubriendo, Colapso por destino, Detección de caída y sondeo (`45bb30e`) (+14 more)

### Community 170 - "InformeZModal.tsx"
Cohesion: 0.08
Nodes (26): 1. Informe Z (cierre de turno), 2. Desglose de Ítems en Ticket (`detalle_items`), 4. Decisiones de diseno, API, Cobros de mesa (auto-servidor), Cobros de mostrador (enviados por cliente), Contenido del Informe Z, Cómo se popula (+18 more)

### Community 171 - "valoracion.use-case.ts"
Cohesion: 0.20
Nodes (7): createSchema, ValoracionUseCase, Valoracion, ValoracionStats, CreateValoracionData, IValoracionRepository, SupabaseValoracionRepository

### Community 172 - "Recogida implícita sin toggle — Implementation Plan"
Cohesion: 0.10
Nodes (19): Task 8: Toggles en `empresas`, Recogida implícita sin toggle — Implementation Plan, Task 1: Migración — borrar filas `recogida` y la columna del toggle, Task 2: Zod — `createModalidadEntregaSchema` solo acepta domicilio, Task 4: Dominio y DTOs — retirar `recogidaTiendaHabilitada`, Task 6: `TiendaFulfillmentSelector` — lista única, recogida fija primero, Task 8: Verificación final, Task 2: Tipos de dominio, DTO Zod y contrato del repositorio (+11 more)

### Community 173 - "next"
Cohesion: 0.07
Nodes (33): next, enviarEmailSchema, generateOrderEmail(), OrderItem, POST(), PATCH(), updateHorasSchema, DELETE() (+25 more)

### Community 174 - "File Structure"
Cohesion: 0.18
Nodes (10): Contexto para quien ejecute esto, File Structure, Fuera de alcance de esta etapa, Landing Page — Etapa 2: Capa de datos de secciones — Implementation Plan, Task 1: Migración — tabla `empresa_landing_secciones`, Task 2: Entidad de dominio `LandingSeccion`, Task 3: DTOs Zod por tipo de sección, Task 4: Repositorio (`ILandingSeccionRepository` + implementación Supabase) (+2 more)

### Community 175 - "Waiter — Sistema de Pases por Ítem"
Cohesion: 0.10
Nodes (20): 1. Waiter añade ítem a la carta (suplantando mesa), 2. Pase propagado desde JSONB, 3. Camarero gestiona pases en `/waiter/pendientes`, API, `/api/pedidos/route.ts`, Base de datos, `cart-drawer.tsx`, Columna `pase` en `pedido_item_estados` (nivel ítem) (+12 more)

### Community 176 - "TPV — Cumplimiento Legal y Normativo"
Cohesion: 0.09
Nodes (21): 10. Backup Fiscal Local (Electron), 1.1 Inalterabilidad e Integridad de registros, 1.2 Cadena de Hashes (Trazabilidad), 1.3 Accesibilidad para inspectores (Volcado de datos), 1.4 Código QR / URL de verificación AEAT, 1.5 Declaración de Responsabilidad del fabricante, 1.5b Declaración modo No-VeriFactu (Art. 12 RD 1007/2023) — **Fase 1 completada**, 1. Ley Antifraude + Reglamento Verifactu (RD 1007/2023) (+13 more)

### Community 177 - "Mejoras de UI y simplificación de recogida — Implementation Plan"
Cohesion: 0.22
Nodes (8): Mejoras de UI y simplificación de recogida — Implementation Plan, Task 1: Migración — recogida siempre a precio 0 en la DB, Task 2: Zod — rechazar precio distinto de 0 en modalidades de recogida, Task 3: Extraer el mapeo de iconos a un módulo compartido, Task 4: Admin — ocultar el precio para modalidades de recogida, Task 5: `TiendaFulfillmentSelector` — icono visible, lista de domicilio rediseñada, recogida como línea fija, Task 7: Colores del panel admin — tema oscuro fijo, igual que la sección de Glovo, Task 8: Verificación final

### Community 178 - "superadmin/page.tsx"
Cohesion: 0.27
Nodes (9): EmpresasTable(), Empresa, EmpresaStats, getData(), getLandingActivas(), getPositionClasses(), GlobalStats, SuperAdminPage() (+1 more)

### Community 179 - "StarRating"
Cohesion: 0.29
Nodes (7): StarRating(), getValueFromX(), handleMouseMove(), handleMouseUp(), handleTouchEnd(), handleTouchMove(), StarRatingProps

### Community 180 - "menu-engineering/page.tsx"
Cohesion: 0.26
Nodes (9): classifyBcg(), computeMedian(), MenuEngineeringPage(), resolveQuadrant(), OcupacionPage(), PeriodPicker(), PeriodPickerProps, PeriodState (+1 more)

### Community 181 - "Propina (Tip) en Mesa"
Cohesion: 0.13
Nodes (14): API, Base de datos, División personalizada (modo `personalizado`), Flujo de pago, `GET /api/mesas/[mesaId]/orders` — respuesta, i18n, `mesa_sesiones` (delta), Migración (+6 more)

### Community 182 - "MenuPanel.tsx"
Cohesion: 0.17
Nodes (13): AddItemPayload, buildComplementMaps(), ComplementDialog(), ComplementDialogProps, ComplementDialogState, fmt(), matchesSearch(), MenuPanel() (+5 more)

### Community 183 - "getSupabaseAnonClient"
Cohesion: 0.06
Nodes (42): ¿Esto es "seguridad"?, Admin UI, Arquitectura de capas, Cómo se integra al catálogo público, Dos vías de escritura — no confundir, Gotcha general descubierto en el camino (no específico de este feature), Menús Virtuales, Qué es (+34 more)

### Community 184 - "logger"
Cohesion: 0.24
Nodes (9): POST(), verifyGlovoSignature(), GlovoStatus, GlovoWebhookPayload, mapGlovoStatusToPedidoEstado(), processGlovoWebhookUseCase(), logger, { fakeSupabase, setFindResult, setUpdateResult } (+1 more)

### Community 185 - "movimientos/page.tsx"
Cohesion: 0.28
Nodes (6): buildQueryString(), FiltersState, formatDate(), MovimientosPage(), TIPO_COLORS, TIPOS

### Community 186 - "Motion Design"
Cohesion: 0.22
Nodes (8): Duration: The 100/300/500 Rule, Easing: Pick the Right Curve, Motion Design, Perceived Performance, Performance, Reduced Motion, Staggered Animations, The Only Two Properties You Should Animate

### Community 187 - "QR Session Enforcement — Mesa Ordering"
Cohesion: 0.12
Nodes (17): API Routes, Clean Architecture Flow, Client Components, Database Schema, Design Rationale, Known Limitations, `mesa_client_tokens`, Middleware: `validateMesaClientToken` (+9 more)

### Community 188 - "orders/page.tsx"
Cohesion: 0.40
Nodes (3): dynamic, metadata, Props

### Community 189 - "main.ts"
Cohesion: 0.11
Nodes (19): blockDangerousShortcuts(), checkForPortableUpdate(), createWindow(), EmpleadoIdSchema, FiscalSnapshotPayload, FiscalSnapshotSchema, isNewerVersion(), PinCacheSchema (+11 more)

### Community 190 - "Frontend Aesthetics Guidelines"
Cohesion: 0.22
Nodes (9): Color & Theme, Frontend Aesthetics Guidelines, Interaction, Layout & Space, Motion, Responsive, Typography, UX Writing (+1 more)

### Community 191 - "Ingrediente"
Cohesion: 0.27
Nodes (4): Ingrediente, mapIngrediente(), mapMerma(), SupabaseStockRepository

### Community 192 - "Mesa Ordering — QR Table Ordering"
Cohesion: 0.12
Nodes (17): API Routes, Component: `MesaOrderHistory`, Component: `MesaOrdersClient`, Customer Flow, Database Schema, `empresas` (delta), `GET /api/mesas/{mesaId}/orders`, `GET /api/mesas?token={uuid}` (+9 more)

### Community 193 - "descuento.use-case.ts"
Cohesion: 0.33
Nodes (7): buildEmailHtml(), DescuentoUseCase, generateCodigo(), getEmailSubject(), getBrevoApiKey(), sendEmail(), SendEmailParams

### Community 194 - "PWA & Service Worker — Waiter Panel"
Cohesion: 0.10
Nodes (20): Architecture, Cache Strategies, CacheFirst — `/_next/static/`, Capacitor Android — IMPLEMENTADO, Critical Gotchas, DevTools 0-byte display bug, GET-only guard, Key Files (+12 more)

### Community 197 - "version/route.ts"
Cohesion: 0.40
Nodes (5): GET(), getTpvRelease(), GithubAsset, GithubRelease, parsed

### Community 198 - "Step 2: Ask UX-Focused Questions"
Cohesion: 0.25
Nodes (7): Accessibility & Inclusion, Aesthetic Preferences, Brand & Personality, Step 1: Explore the Codebase, Step 2: Ask UX-Focused Questions, Step 3: Write Design Context, Users & Purpose

### Community 199 - "PdfRenderer.ts"
Cohesion: 0.21
Nodes (12): @react-pdf/renderer, C, calcTotales(), CONTRATO_LABEL, EmpresaInfo, ExportRow, FichajesDocument(), fmtDate() (+4 more)

### Community 200 - "empresa-config-toggles.spec.ts"
Cohesion: 0.36
Nodes (7): adminEmail(), adminPassword(), EN_RESPUESTA, Interruptor, INTERRUPTORES, motivoParaSaltar(), mutacionesPermitidas()

### Community 201 - "modalidad-entrega.dto.ts"
Cohesion: 0.47
Nodes (4): baseModalidadEntregaSchema, createModalidadEntregaSchema, modalidadEntregaIdSchema, updateModalidadEntregaSchema

### Community 204 - "registerManualMesaPaymentUseCase.ts"
Cohesion: 0.22
Nodes (13): Auto-close on Payment, aplicarCobro(), asentarResultado(), cargarSesionCobrable(), cobrarParteIgual(), cobrarTurnoPersonalizado(), Cobro, fallo() (+5 more)

### Community 207 - "(protected)/page.tsx"
Cohesion: 0.20
Nodes (10): AdminDashboard(), AdminMinimo, configDeEmpresa(), ConfigEmpresa, dynamic, DashboardPedido, DashboardPromoSummary, DashboardStats (+2 more)

### Community 208 - "laborcontrol/export/route.ts"
Cohesion: 0.24
Nodes (9): GET(), GET(), ExportQueryDto, ExportQuerySchema, ResumenParcialQueryDto, ResumenParcialQuerySchema, getExportRepo(), getLcGenerarExportUseCase() (+1 more)

### Community 209 - "db-smoke.spec.ts"
Cohesion: 0.15
Nodes (3): serviceHeaders(), serviceKey(), dotenv

### Community 210 - "Patrones de API Admin — multi_shop"
Cohesion: 0.18
Nodes (12): Añadir una nueva ruta `/api/admin/*`, Constantes de dominio — no hardcodear magic numbers, `getMesaOrdersUseCase` — extracción de lógica compleja, `handleResult()` — mapeo automático de Result<T> a HTTP, `isSuperAdmin` — acceso cross-tenant, Patrones de API Admin — multi_shop, Patrón de uso (handler típico), Qué hace (+4 more)

### Community 211 - "extract/SKILL.md"
Cohesion: 0.33
Nodes (5): Discover, Document, Extract & Enrich, Migrate, Plan Extraction

### Community 212 - "SupervisorPanel.tsx"
Cohesion: 0.21
Nodes (8): dynamic, ESTADO_CHIP, ESTADO_DOT, ESTADO_LABEL, fmtHora(), fmtSegundos(), Props, SupervisorPanel()

### Community 213 - "frontend-design/SKILL.md"
Cohesion: 0.33
Nodes (3): Design Direction, Implementation Principles, The AI Slop Test

### Community 214 - "kitchen-bar-csrf.spec.ts"
Cohesion: 0.33
Nodes (4): DELETE_ROUTES, GET_EXEMPT_ROUTES, PATCH_ROUTES, POST_ROUTES

### Community 215 - "EmpresaPublic"
Cohesion: 0.18
Nodes (11): HeroBannerProps, LandingHeaderProps, LandingPageProps, RenderContext, RedesProps, VisitanosSectionProps, SiteFooterProps, SiteHeaderClientProps (+3 more)

### Community 216 - "confirm-pedido/route.ts"
Cohesion: 0.83
Nodes (3): GET(), POST(), processAndRedirect()

### Community 218 - "electron/printer/index.ts"
Cohesion: 0.35
Nodes (8): printReceipt(), buildAndPrint(), centsToEur(), padEnd(), padStart(), ReceiptData, electron, ref_node_thermal_printer

### Community 219 - "IAdminRepository.ts"
Cohesion: 0.12
Nodes (16): Task 4: `SupabaseAdminRepository` (panel superadmin), ConfiguracionPage(), dynamic, LoginResult, DEFAULT_EMPRESA_COLORES, ADMIN_ROLE, AdminProfile, AdminWithEmpresa (+8 more)

### Community 220 - "waiter-login-form.tsx"
Cohesion: 0.15
Nodes (13): applyMesaAuthResponse(), handleLogout(), doCloseTable(), clearWaiterMesa(), getMesaColors(), getMesaStatus(), MesaCard(), MesaColors (+5 more)

### Community 221 - "normalize/SKILL.md"
Cohesion: 0.50
Nodes (3): Clean Up, Execute, Plan

### Community 222 - "mesa-delete-item-overlay.test.ts"
Cohesion: 0.43
Nodes (7): Task 1: Helpers puros del overlay de borrado optimista, applyPendingDeleteOverlay(), mergeKeyFor(), mergeOrderItems(), OrderItem, withoutPendingDelete(), withPendingDelete()

### Community 223 - "mesa-repository-sessions.test.ts"
Cohesion: 0.39
Nodes (7): consultar(), escenario(), mesaRpc(), crearFakeSupabase(), consumir(), from(), siguienteRespuesta()

### Community 224 - "sw-network-timeout.test.ts"
Cohesion: 0.29
Nodes (3): ref_node_vm, Harness, SW_SOURCE

### Community 225 - "mesas-tenant-header-spoofing.spec.ts"
Cohesion: 0.83
Nodes (3): fetchAnyMesaId(), serviceRoleKey(), supabaseUrl()

### Community 226 - "restore/route.ts"
Cohesion: 0.48
Nodes (6): GET(), getR2Bucket(), getS3Client(), POST(), Snapshot, SnapshotRow

### Community 227 - "mesa-remove-item.test.ts"
Cohesion: 0.29
Nodes (3): ItemReescrito, pedidosDeSesion, updateOrderItemsSpy

### Community 228 - "Capacitor Android PDA — Sistema completo"
Cohesion: 0.25
Nodes (7): Archivos clave, Arquitectura, Capacitor Android PDA — Sistema completo, Estado actual (2026-06-29), Flujo de auto-update, Proceso de build y release — OBLIGATORIO seguir este orden, Variables de entorno y Vercel

### Community 229 - "FichajeEvento"
Cohesion: 0.12
Nodes (12): FichajeConEstado, RegistrarCorreccionInput, RegistrarCorreccionUseCase, derivarEstado(), isOrphan(), RegistrarFichajeInput, IFichajeRepository, RegistrarFichajeInput (+4 more)

### Community 230 - "Database Schema"
Cohesion: 0.25
Nodes (8): Database Schema, `empresas` (delta), `mesa_division_pagos` (nueva tabla), `mesa_sesiones` (delta), `pedidos` (delta — preexistente, usado por esta feature), RPC: `claim_and_create_division_pago(p_sesion_id, p_empresa_id, p_payment_order_ref, p_session_total_cents)`, RPC: `get_mesas_with_sessions(p_empresa_id UUID)`, RPC: `increment_division_pagos(p_sesion_id UUID)`

### Community 231 - "SEO Multi-Tenant"
Cohesion: 0.33
Nodes (5): Archivos clave, Campos de BBDD usados, Features implementadas, SEO Multi-Tenant, Trampas criticas

### Community 232 - "5. API Endpoints"
Cohesion: 0.29
Nodes (7): 5. API Endpoints, Cron Jobs (Vercel), Exportación (Art. 34.9 ET), Fichajes, Integridad, Retenciones Legales, Supervisión

### Community 233 - "Deferred Items ("Para servir después")"
Cohesion: 0.33
Nodes (6): Cart flags, DB storage, Deferred Items ("Para servir después"), `DeferredItem` type, Lifecycle, UI

### Community 234 - "completeCustomPaymentUseCase.ts"
Cohesion: 0.47
Nodes (4): paramsSchema, POST(), CompleteCustomPaymentResult, completeCustomPaymentUseCase()

### Community 235 - "selection/route.ts"
Cohesion: 0.28
Nodes (7): bodySchema, paramsSchema, PATCH(), selectionItemSchema, SelectionItem, UpdateCustomSelectionInput, updateCustomSelectionUseCase()

### Community 236 - "analytics.use-case.ts"
Cohesion: 0.47
Nodes (4): aggregateKpis(), computeDeltaPercent(), ComparisonPeriodParams, FoodCostAnalyticsResponse

### Community 238 - "Mesa Client Tokens"
Cohesion: 0.40
Nodes (5): Camera permission, Mesa Client Tokens, `mesa_client_tokens` table, Rate limiter, Validation middleware

### Community 239 - "Funciones SECURITY DEFINER — Trampas Críticas"
Cohesion: 0.40
Nodes (5): Funciones de trigger vs. funciones RPC, Funciones SECURITY DEFINER — Trampas Críticas, `get_mi_empresa_id()` — excepción intencional, REVOKE FROM PUBLIC, no FROM anon, `SET search_path` y pgcrypto en Supabase

### Community 240 - "correcciones/route.ts"
Cohesion: 0.50
Nodes (3): POST(), CorreccionBodyDto, CorreccionBodySchema

### Community 241 - "holds/route.ts"
Cohesion: 0.60
Nodes (4): CreateHoldSchema, GET(), POST(), getLcGestionarHoldUseCase()

### Community 242 - "6. Integración con TPV"
Cohesion: 0.29
Nodes (6): 6. Integración con TPV, FichajeDialog, Kiosk (modo principal), Mis Fichajes (empleado), Navegación, TpvLoginForm()

### Community 243 - "Protección CSRF"
Cohesion: 0.50
Nodes (4): Comparación timing-safe, CSRF en rutas de camarero y cocina (`/api/waiter/*`, `/api/kitchen/*`), Mecanismo, Protección CSRF

### Community 244 - "Authentication"
Cohesion: 0.50
Nodes (4): Authentication, Endpoints, Login flow — two steps, Session cookie

### Community 246 - "chain-hash.ts"
Cohesion: 0.67
Nodes (3): ChainHashInput, computeChainHash(), verifyLink()

### Community 251 - "AnalyticsPeriodParams"
Cohesion: 0.18
Nodes (5): 3.4 Comparativa de Períodos (`/admin/analytics/comparativa`), AnalyticsUseCase, AnalyticsPeriodParams, IAnalyticsRepository, SupabaseAnalyticsRepository

### Community 263 - "sw.js"
Cohesion: 0.67
Nodes (3): debeCachearse(), networkFirstConTimeout(), TIMEOUT

### Community 275 - "Complejidad cognitiva — cómo se cerró y cómo no volver"
Cohesion: 0.25
Nodes (7): Complejidad cognitiva — cómo se cerró y cómo no volver, Cuándo montar React y cuándo no, Cómo medir (importante), Harness de React — HECHO (2026-08-09), Herramientas ya disponibles, Patrones que han funcionado, Trampas conocidas

### Community 276 - "TPV — Plan: Sistema WAL Local-First y Backup Fiscal"
Cohesion: 0.11
Nodes (17): 1. Electron main process, 2. Renderer (frontend TPV), 3. API Supabase, 4. Electron preload / contextBridge, Archivo físico de cierre de turno, Arquitectura propuesta: Local-First con dos series fiscales, Componentes a implementar, Decisión pendiente — BLOQUEANTE (+9 more)

### Community 278 - "Agentes Relevantes para multi_shop"
Cohesion: 0.12
Nodes (16): 1. Frontend Developer, 2. Backend Architect, 3. UI Designer, 4. Code Reviewer, 5. SEO Specialist, 6. Growth Hacker, 7. Product Manager, Agentes IA - The Agency (+8 more)

### Community 279 - "Fichaje Digital — Registro de Jornada"
Cohesion: 0.11
Nodes (18): Archivos clave, Ciclo de vida de un fichaje, Conservación legal, Correcciones, ¿Cómo funciona?, Estado derivado (panel supervisor), Fichaje Digital — Registro de Jornada, La cadena de integridad (+10 more)

### Community 280 - "Documentación: Toggle Comida/Bebidas en el Menú"
Cohesion: 0.12
Nodes (16): Admin — Panel de Categorías, Admin — Panel de Productos, Archivos clave, Base de datos, Cascade automático, `categorias` (delta), Clasificación de categorías, Comportamiento del toggle (+8 more)

### Community 281 - "TPV — Pases / Marchas en KDS"
Cohesion: 0.12
Nodes (16): API, Base de datos, Componentes afectados, ExistingOrder (MostradorClient.tsx), Flujo, GET /api/tpv/pedidos, GET /api/waiter/kitchen/items, KitchenItem (waiter/kitchen/page.tsx) (+8 more)

### Community 282 - "Welcome Discount - Sistema de Descuento de Bienvenida"
Cohesion: 0.12
Nodes (15): API Endpoints, Base de Datos, Configuración Admin, Códigos de Error, Flujo, Overview, POST `/api/descuento/subscribe`, POST `/api/descuento/validate` (+7 more)

### Community 283 - "Contexto del Proyecto"
Cohesion: 0.14
Nodes (14): Admin (panel protegido), Archivos clave, Banner flotante en modo superadmin, Cliente (público), Contexto del Proyecto, Dominios configurados, Flujo de Datos Principal, Flujo de edición de empresa (+6 more)

### Community 284 - "Sentry — Error Monitoring y Observabilidad"
Cohesion: 0.12
Nodes (15): Alertas, Archivos clave, Arquitectura, Contexto multi-tenant, CSP, Double-channel en ErrorLogger, Error pages de Next.js, Inicialización por runtime (+7 more)

### Community 285 - "Stock, Mermas & Inventario Físico — Sistema de Inventario TPV"
Cohesion: 0.15
Nodes (12): API Routes, Capas, Componentes, Flujo de 3 pasos, Inventario Físico a Ciegas, Migraciones (orden obligatorio), Propósito, Rutas (+4 more)

### Community 286 - "TPV Catalog Cache — Navegación Instantánea + Offline Resilience"
Cohesion: 0.12
Nodes (15): Archivos clave, Arquitectura, Base de datos local: `tpv_catalog`, Endpoints de refresco, Flujo de hidratación, Interfaz del contexto, Phase 2 — IndexedDB (Offline Resilience), Por qué el layout no re-ejecuta (+7 more)

### Community 287 - "Waiter Ticket & Payment UX"
Cohesion: 0.12
Nodes (16): Cancel button, Cerrar Mesa Guard (Waiter Grid), Complementos display, Critical: order estados are synthesized, Custom Payment Selection (CustomSelectionView), How it works, Item Grouping, Kitchen Page — Auto-Collapse on Arrival (+8 more)

### Community 288 - "Waiter Validation Flow"
Cohesion: 0.18
Nodes (11): Confirm buttons (per mesa), Deferred items (bebidas) in validation context, New Endpoints (PR3), Overview, Pendientes Page (`/waiter/pendientes`), Removed Endpoints, Retenidos in the Waiter Grid, Row interaction (+3 more)

### Community 289 - "tpv/pedidos/route.ts"
Cohesion: 0.18
Nodes (15): bodySchema, buildOverridesByPedido(), buildSynthesizedEstado(), GET(), isRetenidoReadyToSynthesize(), ItemEstadoRow, itemSchema, normalizePedidoOrder() (+7 more)

### Community 291 - "tgtg-reserva-popup.tsx"
Cohesion: 0.21
Nodes (12): esAviso(), EstadoAviso, haExpiradoLaRecogida(), MENSAJE_AVISO, ModoAviso, modoTrasReservar(), motivoDeRechazo(), PopupState (+4 more)

### Community 292 - "useCommandQueue.ts"
Cohesion: 0.29
Nodes (14): useCommandQueue(), handleResume(), enqueueCommand(), flushCommandQueue(), getQueuedCommands(), getQueuedCount(), isExpired(), isResumeSignal() (+6 more)

### Community 293 - "vista-mesa.ts"
Cohesion: 0.15
Nodes (13): Mesa/Cobros — Modulos con contrato congelado en tests, ContextoVistaMesa, EstadoTurno, Regla, REGLAS, SesionParaVista, TurnoPersonalizado, VistaMesa (+5 more)

### Community 294 - "Food Cost Analytics — Contexto"
Cohesion: 0.14
Nodes (13): `analytics_food_cost_real`, `analytics_food_cost_teorico`, `analytics_margen_productos`, CMP — Coste Medio Ponderado, Cómo se actualiza, Dónde vive, Food Cost Analytics — Contexto, Fórmula (+5 more)

### Community 295 - "Acuerdo de Tratamiento de Datos (DPA) — Modelo Art. 28 RGPD"
Cohesion: 0.13
Nodes (14): 10. FIRMA, 1. OBJETO Y ALCANCE, 2. INSTRUCCIONES DE TRATAMIENTO, 3.1 Técnicas, 3.2 Organizativas, 3. MEDIDAS DE SEGURIDAD (Art. 32 RGPD), 4. SUBENCARGADOS, 5. DERECHOS DE LOS INTERESADOS (+6 more)

### Community 296 - "DeltaCard.tsx"
Cohesion: 0.25
Nodes (10): Trampas (para sesiones futuras), DELTA_BG, DELTA_COLOR, DeltaCard(), DeltaCardProps, DeltaClass, formatDelta(), formatValue() (+2 more)

### Community 297 - "analytics-types.ts"
Cohesion: 0.42
Nodes (7): FoodCostData, CierreTurnoMovimientoStock, CierreTurnoTopProducto, FoodCostRealRow, FoodCostTeoricoRow, MargenProductoRow, RentabilidadResponse

### Community 299 - "Sistema de Alérgenos por Producto"
Cohesion: 0.15
Nodes (12): `AllergenBadges`, `AllergenIcon`, `AllergenList`, Almacenamiento en DB, Arquitectura de capas, Componentes, Códigos válidos (14 alérgenos EU), Qué es (+4 more)

### Community 300 - "Sistema de Compras y Proveedores (SIALTI) — Contexto"
Cohesion: 0.15
Nodes (12): Archivos clave, Códigos de error COMPRAS_*, Extensiones a tablas existentes, Flujo de pago por caja (`registrarPagoFactura`), IGIC (Canarias), IVA 0%, Paginas Admin, RPC: `recibir_albaran_transaccional` (+4 more)

### Community 301 - "TPV — Sistema de empleados con PIN y permisos"
Cohesion: 0.17
Nodes (11): Admin panel: gestión de empleados, Archivos clave, Arqueo ciego para cajero, Cookie: `tpv_employee_token`, Flujo de autenticación, Permisos por rol, Proxy (`src/proxy.ts`), Qué es (+3 more)

### Community 303 - "pin-cache.ts"
Cohesion: 0.25
Nodes (11): bcryptjs, AttemptRecord, attempts, cachePin(), clearPinCache(), getPinStore(), incrementAttempts(), isRateLimited() (+3 more)

### Community 304 - "Marketing — Flujo del Sistema"
Cohesion: 0.15
Nodes (12): 1. Descubrimiento — Cómo llega el cliente, 2. Captación — Alta del cliente, 2a. Al hacer un pedido *(flujo principal)*, 2b. Pop-up de suscripción *(propuesto — no implementado aún)*, 3. Fidelización — Canales de marketing, 3a. Promociones, 3b. TooGoodToGo (Magic Bags), 4. Gestión — Panel de administración (+4 more)

### Community 305 - "fichajes/page.tsx"
Cohesion: 0.18
Nodes (13): dynamic, FichajesPage(), FichajeTipo, formatEvent(), HISTORY_TIPO_LABEL, initials(), KioskState, RecienteFichaje (+5 more)

### Community 307 - "WaiterBanner"
Cohesion: 0.11
Nodes (18): Realtime — Patrones Criticos (Waiter System), 1. React StrictMode double-mount con canal de nombre fijo, 2. `postgres_changes` silenciado en cliente singleton, 3. Race condition en validate loop, 4. Mesa grid badge no se actualiza al marcar items en cocina, 7. Canal de Broadcast = clave de ruteo, no identificador libre, Anon y RLS — regla de oro (desde 2026-07-31), Arquitectura de canales activa (+10 more)

### Community 308 - "Row Level Security (RLS)"
Cohesion: 0.17
Nodes (12): Causa raíz encontrada — `ALTER DEFAULT PRIVILEGES` en `public` (2026-07-31, tercera pasada), Cuarta pasada (auditoría externa, 2026-07-31) — GRANTs heredados en las 53 tablas ya existentes, Función de auditoría — `check_public_function_grants()`, Función de auditoría — `check_rls_policy_hygiene()`, Incidente 2026-07-31 — fuga cross-tenant vía RLS PERMISSIVE + Realtime, Lecturas públicas, Otros defaults inseguros de Postgres cubiertos (misma meta-revisión), Políticas de denegación anónima (RESTRICTIVE) (+4 more)

### Community 309 - "rentabilidad/page.tsx"
Cohesion: 0.24
Nodes (10): formatCents(), getMonthRange(), getWeekRange(), PeriodType, RentabilidadPage(), resolveMarginClass(), SortDir, SortField (+2 more)

### Community 310 - "cierre-report-view.tsx"
Cohesion: 0.14
Nodes (15): 3.1 Matriz BCG de Menú (`/admin/analytics/menu-engineering`), 3.2 Heatmap de Ocupación (`/admin/analytics/ocupacion`), 3.3 Informe de Cierre de Turno (`/tpv/analytics/cierre/[turnoId]`), Analítica Avanzada — Contexto Técnico, Componentes Compartidos, Migraciones, Resumen, CierreReportView() (+7 more)

### Community 311 - "laborcontrol/offline-queue.ts"
Cohesion: 0.21
Nodes (14): FichajeDialog(), Props, RGPD_KEY(), TIPO_LABEL, b64(), enqueue(), fromB64(), getOrCreateKey() (+6 more)

### Community 312 - "Product"
Cohesion: 0.09
Nodes (25): Sistema de Alergenos, Trampas conocidas, Segunda imagen de producto (tiendas) Implementation Plan, Task 10: Full verification pass, Task 1: Database migration — `foto_url_2` column, Task 2: Domain type + Zod DTOs, Task 3: Repository — interface + Supabase implementation, Task 4: Public view model — `MenuItemVM.image2` (+17 more)

### Community 313 - "Diseño: mejoras de UI y simplificación de "recogida" en modalidades de entrega"
Cohesion: 0.22
Nodes (8): 1. Recogida pasa a ser SIEMPRE gratis, en todo el sistema, 2. `TiendaFulfillmentSelector.tsx` — rediseño visual (opción B + A del brainstorming), 3. Colores del panel admin — alinear con el tema oscuro fijo existente, Contexto, Decisiones (aprobadas visualmente por el usuario), Diseño: mejoras de UI y simplificación de "recogida" en modalidades de entrega, Fuera de alcance, Testing

### Community 314 - "Imágenes — subida, optimización y coste"
Cohesion: 0.20
Nodes (9): Cuándo SÍ usar `next/image` normal, Cómo se usa, El banner es un caso aparte, El guard, y por qué existe, Imágenes — subida, optimización y coste, Lo que costaba de verdad, Por qué NO se usa `next/image` con esas imágenes, Si vuelve a subir el consumo (+1 more)

### Community 315 - "supabase-mesa-client-token.repository.ts"
Cohesion: 0.09
Nodes (18): Domain Layer, `IMesaClientTokenRepository`, `MesaClientTokenUseCase`, Adenda (post-implementación), Alcance, Cambios, Cart Access Token — Eliminación del flujo legacy, Contexto (+10 more)

### Community 317 - "empresa"
Cohesion: 0.09
Nodes (26): Context, Contexto para quien ejecute esto, File Structure, Fuera de alcance de esta etapa, Landing Page — Etapa 4: Renderizado público data-driven — Implementation Plan, Task 1: `readTranslatable` — helper de lectura i18n, Task 4: Conectar `src/app/page.tsx` + limpiar código muerto, Task 5: Verificación final (+18 more)

### Community 318 - "TiendaFulfillmentSelector.tsx"
Cohesion: 0.06
Nodes (40): Fase 3 — Catálogo público y extracción de Mapbox, Task 10: `/admin/delivery` condicional por tipo, Task 11: Extraer `MapboxAddressInput` de `DeliveryMethodSelector`, Task 13: `TiendaFulfillmentSelector`, Task 17: Suite completa y traducciones, 10. Fuera de alcance / futuro, 2. Decisión de arquitectura, 5. Admin UI (+32 more)

### Community 320 - "Sistema de Complementos por Producto"
Cohesion: 0.25
Nodes (7): API Routes, Arquitectura de capas, Cómo se guardan en los pedidos, Qué es, Sistema de Complementos por Producto, Tablas DB, Trampas críticas

### Community 321 - "HeatmapGrid.tsx"
Cohesion: 0.24
Nodes (9): buildCellMap(), DAY_LABELS, DAYS, HeatmapGrid(), HeatmapGridProps, HOURS, normalizeHeatmapCell(), resolveMax() (+1 more)

### Community 322 - "proxy-autorizacion.test.ts"
Cohesion: 0.20
Nodes (6): isTokenRevokedMock, jwtVerifyMock, rateLimitAdminMock, verifyCsrfTokenMock, verifyTpvEmployeeTokenMock, verifyWaiterTokenMock

### Community 325 - "Mesa Payments — Pagar en mesa con Redsys"
Cohesion: 0.06
Nodes (36): Activación, Archivos, Archivos involucrados, Cancelación, Diferencia pago total vs. división, Dos paths en el webhook, En el menú (`/?mesa={token}`), En el ticket (`/mesa/{mesaId}/orders`) (+28 more)

### Community 326 - "Atomicidad del Pago Completo (implementada 2026-07-30)"
Cohesion: 0.22
Nodes (9): Atomicidad del Pago Completo (implementada 2026-07-30), Hardening REST API (migración `20260730000004`), Pieza 1 — Trigger `check_session_not_locked` (migración `20260730000002`), Pieza 2 — RPC `initiate_mesa_payment_atomic` (migración `20260730000003`), Pieza 3 — Cambios en `initiateRedsysMesaPaymentUseCase.ts`, Pieza 4 — Captura de `PAYMENT_IN_PROGRESS` en `POST /api/pedidos`, Por qué SERIALIZABLE no funciona aquí, Solución: bloqueo pesimista sobre la fila padre (+1 more)

### Community 328 - "rls-policy-hygiene.spec.ts"
Cohesion: 0.28
Nodes (7): fetchHygieneRows(), HygieneCheckName, HygieneViolation, INTENTIONAL_DEFAULT_PRIVILEGE_GRANTORS, INTENTIONAL_PUBLIC_TRUE_TABLES, serviceRoleKey(), supabaseUrl()

### Community 330 - "getPedidoRepository"
Cohesion: 0.08
Nodes (24): dynamic, PATCH(), VALID_ESTADOS, dynamic, GET(), dynamic, GET(), dynamic (+16 more)

### Community 332 - "secrets-scan.test.ts"
Cohesion: 0.25
Nodes (8): ELECTRON_DIR, EXCLUDED_PATHS, getSourceFiles(), INCLUDED_EXTENSIONS, ROOT, SECRET_PATTERNS, SRC_DIR, walkDir()

### Community 333 - "API Routes"
Cohesion: 0.25
Nodes (8): API Routes, `DELETE /api/mesas/{mesaId}/division`, `DELETE /api/mesas/{mesaId}/lock`, `GET /api/mesas/{mesaId}/orders`, `GET /api/redsys/cancel-mesa?mesaId={uuid}&redirect={path}`, `POST /api/mesas/{mesaId}/division`, `POST /api/mesas/{mesaId}/lock`, `POST /api/redsys/initiate-mesa`

### Community 335 - "isTokenRevoked"
Cohesion: 0.46
Nodes (6): jose, POST(), getRedisConfig(), isTokenRevoked(), redisRequest(), revokeToken()

### Community 336 - "food-cost/page.tsx"
Cohesion: 0.43
Nodes (7): FoodCostPage(), formatCents(), getMonthRange(), getWeekRange(), PeriodType, resolveDeviationClass(), resolveDeviationLabel()

### Community 338 - "Estándares y certificaciones de seguridad — referencia"
Cohesion: 0.29
Nodes (7): Estándares y certificaciones de seguridad — referencia, GDPR / LOPDGDD ✅ (aplica), ISO 27001, Ley Antifraude — RD 1007/2023 ✅ (aplica al TPV), OWASP Top 10, PCI DSS (Payment Card Industry Data Security Standard), SOC 2 (Service Organization Control 2)

### Community 339 - "analitica-vercel.tsx"
Cohesion: 0.33
Nodes (8): @vercel/analytics, @vercel/speed-insights, AnaliticaVercel(), esNavegadorAutomatizado(), esRutaDePersonal(), filtrarEventoDeAudiencia(), rutaDe(), RUTAS_DE_PERSONAL

### Community 340 - "Sub-features"
Cohesion: 0.29
Nodes (6): 5.1 Exportación a Contabilidad, 5.2 Conexión con Deliveroo / Just Eat / Uber Eats, 5.3 Reservas Online, Bloque 5 — Integraciones Externas, Sub-features, TPV & F&B — Roadmap de Funcionalidades Futuras

### Community 341 - "supabase-security-definer.spec.ts"
Cohesion: 0.29
Nodes (3): INTENTIONAL_ANON_WHITELIST, INTENTIONAL_AUTHENTICATED_WHITELIST, RPC_FUNCTIONS_MUST_BE_BLOCKED

### Community 343 - "MenuCategoryVM"
Cohesion: 0.09
Nodes (36): Menús Virtuales, Mapper — `toVirtualCategoryVM` / `toVirtualSubcategoryVM`, Testing, Fase 3: orden combinado entre categorías reales y menús virtuales, Self-review notes, Task 10: Extend `GetMenuUseCase` and its wiring, Task 22: Propagar `orden` a `MenuCategoryVM`, Task 23: Mezclar categorías reales y menús virtuales por `orden` (+28 more)

### Community 349 - "Prompt: Búsqueda de Clientes Potenciales — multi_shop"
Cohesion: 0.33
Nodes (5): Filtros de selección (MUY IMPORTANTE), Información a recopilar por negocio, Objetivo, Prompt: Búsqueda de Clientes Potenciales — multi_shop, Reglas importantes

### Community 350 - "perfil-laboral.dto.ts"
Cohesion: 0.33
Nodes (5): CreatePerfilLaboralDto, CreatePerfilLaboralSchema, TipoContratoEnum, UpdatePerfilLaboralDto, UpdatePerfilLaboralSchema

### Community 353 - "fuzz-api-inputs.test.ts"
Cohesion: 0.13
Nodes (10): fast-check, abrirTurnoSchema, arbCorruptValue, cobroSchema, fichajeKioskSchema, rectificarSchema, arbImporte, arbIva (+2 more)

### Community 356 - "Skill Registry — multi-shop"
Cohesion: 0.40
Nodes (4): Compact Rules (inject into sub-agents), Project Context, Skill Registry — multi-shop, User Skills

### Community 357 - ".verifyToken"
Cohesion: 0.15
Nodes (14): Seguridad (Checklist Critico), Aplicación en routes, Autenticación y sesión, Control de acceso basado en roles (RBAC), Fail-closed en producción, Flujo de autenticación, JWT en cookies HttpOnly, JWT Revocation (+6 more)

### Community 360 - "anon-realtime-column-privileges.spec.ts"
Cohesion: 0.50
Nodes (3): anonHeaders(), anonKey(), TABLES

### Community 363 - "ref_node_fs"
Cohesion: 0.14
Nodes (12): ref_node_fs, ref_node_path, API_DIR, EXCLUDED_PATHS, ROOT, walkDir(), MAIN_TS, PRELOAD_TS (+4 more)

### Community 383 - "mermas/page.tsx"
Cohesion: 0.23
Nodes (9): buildEmptyForm(), fetchIngredientes(), fetchTurno(), FormState, MermasPage(), handleSubmit(), MOTIVOS, submitMerma() (+1 more)

### Community 384 - "successResponse"
Cohesion: 0.07
Nodes (43): Fail-closed en login (producción), Nivel handler (defense in depth), Nivel proxy (antes de JWT verification), Rate Limiting, Trampas conocidas, uuid, GET(), POST() (+35 more)

### Community 543 - "usePagoDeMesa"
Cohesion: 0.17
Nodes (13): Complejidad Cognitiva (S3776) — Como se cerro y como no volver, Cerradas — lo que dejó cada una, El hook que faltaba — `usePagoDeMesa` (2026-08-09), `mesa-orders-client.tsx` — de 78 a 14 (2026-08-09), BotonDePago(), buildTotalMismatch(), getExpectedCents(), getStoredMismatch() (+5 more)

## Knowledge Gaps
- **2238 isolated node(s):** `@upstash/context7-mcp`, `@playwright/mcp`, `graphify-mcp`, `@supabase/mcp-server-supabase`, `SUPABASE_ACCESS_TOKEN` (+2233 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 2637 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **29 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `next` connect `next` to `successResponse`, `getSupabaseClient`, `app/page.tsx`, `mesa-orders-client.tsx`, `waiter-auth.ts`, `useLanguage`, `tpv-types.ts`, `react`, `handleResult`, `CobroFlow.tsx`, `waiter-pin/route.ts`, `albaranes/[id]/page.tsx`, `database/index.ts`, `getAuthAdminUseCase`, `getMesaSesionUseCase`, `promociones/unsubscribe/route.ts`, `initiateRedsysMesaPaymentUseCase.ts`, `tpv/layout.tsx`, `server-services.ts`, `tpv/pedidos/route.ts`, `app/layout.tsx`, `WaiterLoginForm`, `tgtg-reserva-popup.tsx`, `MostradorClient.tsx`, `getDomainFromHeaders`, `api/pedidos/route.ts`, `tpv-catalog-ctx.tsx`, `promociones/route.ts`, `proxy.ts`, `fichajes/page.tsx`, `delivery/page.tsx`, `waiter-catalog-ctx.tsx`, `cierre-report-view.tsx`, `kitchen/layout.tsx`, `logger`, `language-context.tsx`, `(protected)/layout.tsx`, `orders/page.tsx`, `legal/page.tsx`, `t`, `telegram.service.ts`, `proxy-autorizacion.test.ts`, `version/route.ts`, `cn`, `HistorialClient.tsx`, `getPedidoRepository`, `AnalyticsClient.tsx`, `(protected)/page.tsx`, `isTokenRevoked`, `tgtg/enviar/route.ts`, `laborcontrol/export/route.ts`, `cart-drawer.tsx`, `SupervisorPanel.tsx`, `infrastructure/index.ts`, `confirm-pedido/route.ts`, `pago-ok/page.tsx`, `IAdminRepository.ts`, `MesasGrid.tsx`, `waiter-login-form.tsx`, `restore/route.ts`, `waiter/kitchen/page.tsx`, `completeCustomPaymentUseCase.ts`, `selection/route.ts`, `processRedsysWebhookUseCase`, `package.json`, `correcciones/route.ts`, `holds/route.ts`, `removeSessionItemUseCase.ts`, `tabla-plantillas/route.ts`, `rate-limit.ts`, `pedido.ts`, `mermas/page.tsx`?**
  _High betweenness centrality (0.150) - this node is a cross-community bridge._
- **Why does `Result` connect `Result` to `supabase-empresa.repository.ts`, `ModalidadEntrega`, `getSupabaseClient`, `TgtgPromocion`, `app/kitchen/page.tsx`, `@supabase/supabase-js`, `ComplementoGrupo`, `tpv-types.ts`, `handleResult`, `Category`, `SupabaseTablaPlantillaRepository.ts`, `IClienteRepository`, `entities/types.ts`, `Rediseño del árbol de Menús Virtuales — Implementation Plan`, `waiter-pin/route.ts`, `EmpresaColores`, `getAuthAdminUseCase`, `IStockRepository`, `initiateRedsysMesaPaymentUseCase.ts`, `LandingSeccion`, `SupabaseSuperAdminRepository.ts`, `glovo.service.ts`, `supabase-pedido.repository.ts`, `MenuVirtual`, `analytics-types.ts`, `valoracion.use-case.ts`, `delivery/page.tsx`, `landing-seccion.use-case.ts`, `domain/types.ts`, `Product`, `SupabaseHoldRepository.ts`, `supabase-mesa-client-token.repository.ts`, `supabase-stock.repository.ts`, `Ingrediente`, `HeatmapGrid.tsx`, `descuento.use-case.ts`, `telegram.service.ts`, `promocion.use-case.ts`, `SupabaseExportRepository.ts`, `registerManualMesaPaymentUseCase.ts`, `logger.ts`, `auth-admin.use-case.ts`, `PedidoUseCase`, `IAdminRepository.ts`, `FichajeEvento`, `completeCustomPaymentUseCase.ts`, `selection/route.ts`, `analytics.use-case.ts`, `RecetaItem`, `PerfilLaboral`, `removeSessionItemUseCase.ts`, `SupabaseReviewQueueRepository.ts`, `processRedsysWebhookUseCase.ts`, `AnalyticsPeriodParams`, `rate-limit.ts`?**
  _High betweenness centrality (0.108) - this node is a cross-community bridge._
- **Why does `t()` connect `t` to `fetchWithCsrf`, `app/page.tsx`, `useAdmin`, `app/kitchen/page.tsx`, `mesa-orders-client.tsx`, `useLanguage`, `react`, `(protected)/pedidos/page.tsx`, `albaranes/[id]/page.tsx`, `pendientes/page.tsx`, `json-ld.ts`, `usePagoDeMesa`, `app/layout.tsx`, `Convenciones a seguir (ya presentes en el repo, no las reinventes)`, `tgtg-reserva-popup.tsx`, `bar/page.tsx`, `WaiterBanner`, `menu-engineering/page.tsx`, `rentabilidad/page.tsx`, `movimientos/page.tsx`, `language-context.tsx`, `TiendaFulfillmentSelector.tsx`, `legal/page.tsx`, `CLAUDE.md - Contexto multi_shop`, `cn`, `food-cost/page.tsx`, `cart-drawer.tsx`, `PedidoUseCase`, `waiter-login-form.tsx`, `audit-log/page.tsx`, `waiter/kitchen/page.tsx`, `slider-banner.tsx`, `Seguridad — multi_shop`, `banner-visibilidad.ts`, `CategoryNav`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Are the 14 inferred relationships involving `t()` (e.g. with `SonarLint — Reglas activas (aplicar siempre)` and `UI & Design System (Tailwind v4)`) actually correct?**
  _`t()` has 14 INFERRED edges - model-reasoned connections that need verification._
- **Are the 10 inferred relationships involving `getSupabaseClient()` (e.g. with `Arquitectura y Capas` and `Menús Virtuales`) actually correct?**
  _`getSupabaseClient()` has 10 INFERRED edges - model-reasoned connections that need verification._
- **What connects `@upstash/context7-mcp`, `@playwright/mcp`, `graphify-mcp` to the rest of the system?**
  _2238 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `fetchWithCsrf` be split into smaller, more focused modules?**
  _Cohesion score 0.02707244212098581 - nodes in this community are weakly interconnected._