...
- **`ultima_actividad` en `clientes`** — DEFAULT NOW() al crear. Se actualiza por trigger `trg_pedidos_ultima_actividad` en cada INSERT a `pedidos`. Base del auto-purge.
- **Vercel Cron RGPD** — `GET /api/cron/rgpd-purge`, mensual (día 1, 03:00 UTC). Anonimiza `clientes` con `ultima_actividad < now() - 5 años AND anonimizado_en IS NULL`. Requiere `CRON_SECRET` en Vercel env vars. pg_cron NO disponible (plan Free Supabase). Ver `docs/context/rgpd-clientes.md`.
- **`pedidos` DELETE bloqueado** — trigger `pedidos_no_delete` (migración `20260722000002`). Mismo patrón que `tpv_cobros` y `tpv_turnos`. Art.66 LGT — retención fiscal mínima 5 años.
- **`LegalChainVerify` — exportación para auditores** — `/tpv/legal` tiene selector de año (últimos 5) + rango de fechas personalizado → descarga JSON via `GET /api/tpv/audit/export?desde=&hasta=`. Distinto del `audit_log` admin (operativo — quién hizo qué); este export es para inspectores de Hacienda.
