-- supabase/migrations/20260717000002_analytics_cierre_turno.sql
-- Reporte de cierre de turno TPV: ventas, arqueo, mermas, top productos

CREATE OR REPLACE FUNCTION analytics_cierre_turno(p_turno_id uuid)
RETURNS TABLE (
  turno_id              uuid,
...
BEGIN
    ...
    SELECT
      jsonb_agg(
        jsonb_build_object(
          ...
          'coste', COALESCE(ms.cantidad * ms.precio_unitario_cmp_cents, 0)
        )
      ) AS arr,
      COALESCE(SUM(ms.cantidad * ms.precio_unitario_cmp_cents), 0)::bigint AS total_cents
  ...
  SELECT
    ...
    COALESCE((SELECT total_cents FROM mermas_agg), 0::bigint)
  FROM turno_row tr;
END;
$$;

GRANT EXECUTE ON FUNCTION analytics_cierre_turno(uuid) TO service_role;
GRANT EXECUTE ON FUNCTION analytics_cierre_turno(uuid) TO authenticated;
