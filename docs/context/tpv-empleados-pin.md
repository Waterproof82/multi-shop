...
- **Solo `encargado` puede abrir turno por PIN**: el cajero nunca ve la pantalla de abrir turno. Si llega a ella, el layout lo redirige a `/tpv/turno/espera`. Solo admin y encargado pueden abrir.
- **`csrf_token` DEBE generarse en el login de empleado**: el proxy valida CSRF en todas las mutaciones (POST/PUT/PATCH/DELETE) incluyendo las rutas TPV autenticadas con `tpv_employee_token`. Si el login de empleado no genera la cookie `csrf_token`, todas las mutaciones posteriores devuelven 403 CSRF_REQUIRED. Ver `src/app/api/tpv/empleados/login/route.ts` — usa `generateCsrfToken` + `signCsrfToken` igual que el admin login.
- **Cajero sin turno activo → loop infinito de redirects**: el layout redirige a `/tpv/turno/abrir` cuando no hay turno, y esa página redirige cajeros a `/tpv/mostrador`. El layout evita el loop redirigiendo cajeros a `/tpv/turno/espera` en lugar de `/tpv/turno/abrir`.
- **`/tpv/login` NO redirige por `admin_token`**: la página de login es exclusiva para autenticación PIN. Los admins acceden al TPV directamente (el layout valida el `admin_token`). Solo encargados con token válido son auto-redirigidos a `/tpv/turno/abrir`; cajeros y admins sin sesión de empleado siempre ven el formulario de PIN.
- **Nombre del operador en turno es readonly para empleados**: el campo se pre-rellena con `payload.nombre` del token y no es editable. Garantiza trazabilidad — el turno queda vinculado al empleado autenticado.
