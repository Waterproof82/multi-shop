...
import { createFacturaProveedorUseCase } from '@/core/application/use-cases/compras/factura/createFacturaProveedor.use-case';
...
export async function GET(req: NextRequest) {
  ...
  const result = await getComprasRepository().findFacturas(ctx.empresaId, {
    estadoPago: q.get('estado_pago') ?? undefined,
...
export async function POST(req: NextRequest) {
  ...
  const result = await createFacturaProveedorUseCase(getComprasRepository(), ctx.empresaId, body);
