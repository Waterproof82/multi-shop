...
// Pinga /api/kitchen/items antes de recargar — verifica que el servidor responde,
// no solo que hay red. Android puede disparar 'online' antes de que la red esté estable.
function reloadWhenReady() {
  fetch('/api/kitchen/items', { cache: 'no-store' })
    .then(() => { globalThis.location.reload(); })
    .catch(() => { setTimeout(reloadWhenReady, 2000); });
}

export function KitchenOfflineBanner() {
