...
import { Clock, CheckCircle, AlertCircle, PartyPopper, X } from "lucide-react";
import { getTrackingTokens, removeTrackingToken } from "@/lib/order-tracking";
...
function OrderCard({ order, isPrimary, onDismiss }: { order: OrderState; isPrimary: boolean; onDismiss: () => void }) {
  const { status } = order;
  const ready = isReady(status?.estimated_ready_at ?? null);

  if (order.error) {
    return (
      <div className={`rounded-xl border p-4 flex items-center gap-3 ${isPrimary ? 'border-destructive/40 bg-destructive/5' : 'border-border bg-muted/30'}`}>
        <AlertCircle className="w-5 h-5 text-destructive shrink-0" />
        <p className="text-sm text-muted-foreground flex-1">Pedido no encontrado.</p>
        <button onClick={onDismiss} aria-label="Cerrar" className="hover:opacity-70 transition-opacity">
  ...
  if (!status) {
    return (
      <div className={`rounded-xl border p-4 flex items-center gap-3 ${isPrimary ? 'border-border bg-background' : 'border-border bg-muted/30'}`}>
        <div className="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin shrink-0" />
        <p className="text-sm text-muted-foreground flex-1">Cargando estado...</p>
      </div>
    );
  }
    ...
    <div className={`rounded-xl border p-4 flex items-start gap-3 ${isPrimary ? 'border-primary/30 bg-background' : 'border-border bg-muted/30'}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {status.estimated_minutes === null ? (
          <p className="text-xs text-muted-foreground mt-0.5">Tiempo estimado pendiente de confirmar.</p>
        ) : (
          <div className="flex items-center gap-1 mt-0.5">
            <Clock className="w-3.5 h-3.5 text-primary shrink-0" />
            <p className="text-xs text-muted-foreground">
              {status.estimated_minutes} min
              {status.estimated_ready_at && (
                <> · listo a las <span className="font-medium text-foreground">{formatTime(status.estimated_ready_at)}</span></>
              )}
            </p>
          </div>
        )}
...
export function TrackingPageClient({ token, initialStatus }: TrackingPageClientProps) {
  ...
  // Initialize orders: primary token first, then other tokens from localStorage
  useEffect(() => {
  ...
  const pollAll = useCallback(async () => {
    setOrders(prev => prev.map(o => o)); // trigger re-render for time checks
    const updates = await Promise.all(
      orders.map(async (o) => {
        if (o.error) return o;
        try {
          const res = await fetch(`/api/orders/status?token=${o.token}`);
          if (res.status === 404) return { ...o, error: true };
          if (!res.ok) return o;
  ...
  const dismissOrder = useCallback((dismissToken: string) => {
    removeTrackingToken(dismissToken);
    setOrders(prev => prev.filter(o => o.token !== dismissToken));
  }, []);
              ...
              <div className="rounded-xl bg-secondary px-6 py-5 max-w-sm space-y-3">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className="text-lg font-semibold text-foreground">
                    Tiempo estimado: {primaryOrder.status.estimated_minutes} minutos
                  </span>
                </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {otherOrders.map(order => (
            <OrderCard
              ...
              order={order}
              isPrimary={false}
              onDismiss={() => dismissOrder(order.token)}
            />
          ))}
        </div>
      )}
